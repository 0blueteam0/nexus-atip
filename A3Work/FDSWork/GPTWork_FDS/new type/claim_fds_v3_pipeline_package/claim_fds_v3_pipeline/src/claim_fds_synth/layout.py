...
def _resolve_font_path(path: str) -> str:
    """Return a usable Korean-capable font path across Linux and Windows.

    The package default points at Nanum fonts on Linux. On this Windows/MSYS
    host, keep the configured path when it exists and otherwise fall back to
    installed Korean-capable fonts so baseline rendering remains reproducible.
    """
    original = Path(path)
    if original.exists():
        return str(original)
    candidates = [
        ...
        Path("C:/Windows/Fonts/NotoSerifKR-VF.ttf"),
    ...
    lower = path.lower()
    if "bold" in lower or "bd" in lower:
        candidates = [
            Path("C:/Windows/Fonts/NotoSansKR-VF.ttf"),
            Path("C:/Windows/Fonts/malgunbd.ttf"),
            Path("/c/Windows/Fonts/malgunbd.ttf"),
