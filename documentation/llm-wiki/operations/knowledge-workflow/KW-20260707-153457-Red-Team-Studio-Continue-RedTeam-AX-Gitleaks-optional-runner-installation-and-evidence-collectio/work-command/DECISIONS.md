Decision ledger: Gitleaks is optional runner coverage only; stdout is forced to JSON with log-level error; no secret-like committed positive fixture is used; binaries are not committed, only manifest and hash metadata are tracked.
Evidence fields:
