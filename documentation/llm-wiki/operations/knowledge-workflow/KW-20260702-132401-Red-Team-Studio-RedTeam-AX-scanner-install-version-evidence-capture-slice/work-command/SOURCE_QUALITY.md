---
type: work_command_record
task_id: KW-20260702-132401-Red-Team-Studio-RedTeam-AX-scanner-install-version-evidence-capture-slice
project: Red-Team-Studio
task: RedTeam AX scanner install version evidence capture slice
created: 2026-07-02T13:24:01+09:00
source_package: K:/wiki/work command
---

# SOURCE_QUALITY

## Source Classification

| source | source_content_class | usable_for | reason | required_follow_up |
|---|---|---|---|---|

## Promotion Rule

A source can support a checklist requirement only when it contains actual requirement, procedure, record, control, or evaluation content. Purchase pages, catalog pages, abstracts, and search result metadata are not enough.

## Downgrade Rule

If a source is purchase-only, abstract-only, or search-result-only, record it as discovery evidence only. Do not use it as direct compliance evidence.



## Autofill Work Command Evidence

Source classification: caller-provided local artifacts and command outputs.
Usable for: session handoff and evidence trace.
Not enough for: external factual claims unless paired with source URLs or repository files.
Required follow-up: add source URLs or exact local source paths for research-heavy work.
