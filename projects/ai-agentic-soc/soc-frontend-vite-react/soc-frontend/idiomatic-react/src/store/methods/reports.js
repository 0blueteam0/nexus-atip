...
,
  redTeamAnalysis2Panel() {
    ...
    const runtimeReadinessRows = [
      ...
      ['운영 closure 증거 인증 API', '/api/redteam/v2/toolchains/certify-reviewed-operating-close-evidence', '실제 운영 산출물과 실제 승인자 attestation을 completion audit 후보로 검증합니다'],
      ['운영 completion audit 검토 API', '/api/redteam/v2/toolchains/review-operating-completion-audit-candidate', '인증 후보를 독립 감사 checklist로 다시 검토하고 controlled/test 산출물은 완료 후보에서 차단합니다'],
    ...
    const operatorEvidenceSubmissionManifestRows = (operatorEvidenceSubmissionManifest.attachment_rows || []).map(item => [
      ...
      koValue(item.status || '미확인'),
      [
        item.artifact_path ? `파일: ${item.artifact_path}` : 'artifact_path 없음',
        item.sha256 ? `sha256: ${String(item.sha256).slice(0, 12)}...` : 'sha256 없음',
        `상태: ${item.artifact_status || '-'} / 기대: ${item.expected_status || '-'}`,
    ...
    const realOperatingToolCoverageRows = (realOperatingEvidenceReadiness.tool_coverage || []).map(item => [
      ...
      item.selected_path || 'Nuclei/OpenVAS/Trivy/SCA/npm audit/ZAP 6개 결과가 모두 필요',
      ...
      smallPanel('도구 실행 계획 / 샌드박스 정책',
        h('div', { style:{ display:'grid', gap:'10px' } },
          ...
          h('div', { style:{ borderTop:`1px solid ${C.border}`, paddingTop:'10px', display:'grid', gap:'10px' } },
            ...
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '운영 closure 제출 패키지 API는 /api/redteam/v2/toolchains/operating-closure-submission-package 입니다. source_dir, 승인자 4명, runtime blocker, close-operating payload를 먼저 검증하고 scanner 명령은 실행하지 않습니다. 사람 검토 API는 /api/redteam/v2/toolchains/operating-closure-human-review 이며, 검토 완료 실행 API는 /api/redteam/v2/toolchains/execute-reviewed-operating-close 입니다. 증거 인증 API는 /api/redteam/v2/toolchains/certify-reviewed-operating-close-evidence 입니다.'),
            ...
            h('div', { style:{ borderTop:`1px solid ${C.border}`, paddingTop:'8px', display:'grid', gap:'8px' } },
              ...
              h('div', { style:{ fontSize:'10px', color:C.sec, lineHeight:1.45 } }, '이미 사람이 승인 범위에서 실행해 저장한 scanner 산출물 폴더만 사용합니다. 먼저 운영 closure 제출 패키지를 만들고, 사람 검토 기록으로 체크리스트와 승인자 서명을 남긴 뒤 전체 닫기를 실행합니다. 이 경로는 도구 명령을 실행하지 않습니다.'),
