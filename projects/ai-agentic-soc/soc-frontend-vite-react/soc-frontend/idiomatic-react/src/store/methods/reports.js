    ...
    return h('div', { style:{ display:'grid', gap:'14px' } },
      ...
      smallPanel('도구 래퍼 신뢰 고정 / 버전 확인',
        h('div', { style:{ display:'grid', gap:'10px' } },
          ...
          this.renderTable(['도구','고정 상태','사용 가능 여부','해시/위치'], wrapperRows.length ? wrapperRows : [['미로드','-','-','상태 새로고침 필요']]))),
      ...
      smallPanel('OpenVAS/ZAP 서비스 결과 가져오기',
        h('div', { style:{ display:'grid', gap:'10px' } },
          ...
          serviceImportArtifact.storage_path ? h('div', { style:{ fontSize:'9.5px', color:C.sec } }, '서비스 결과 원본은 분석 저장소에 보관됨') : null)),
      smallPanel('분석가용 다음 실행 안내',
        h('div', { style:{ display:'grid', gap:'10px' } },
          h('div', { style:{ fontSize:'11px', color:C.sec, lineHeight:1.55 } },
            '분석가는 아래 순서대로 버튼을 누르면 됩니다. 컨테이너, 보조 실행 환경, 읽기 전용 접속 주소 같은 분석 환경 설정은 아래 관리자용 영역에서 따로 확인합니다. 고위험 실행은 사람이 승인한 뒤 진행합니다.'),
      ...
      smallPanel('도구 실행 계획 / 샌드박스 정책',
        h('div', { style:{ display:'grid', gap:'10px' } },
          ...
          h('div', { style:{ borderTop:`1px solid ${C.border}`, paddingTop:'10px', display:'grid', gap:'10px' } },
            ...
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '복합 Collection 전체 닫기는 명시된 사람 승인자 정보를 받아 Evidence 승인, Finding 승격, 2인 severity 승인, Matrix, Report v2 draft, export 승인, export, completion gate를 순서대로 수행하지만 분석도구 명령과 능동 스캔은 실행하지 않습니다.'),
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '운영 closure 제출 패키지는 운영 산출물 폴더, 승인자 4명, 실행 차단 조건, 운영 닫기 요청을 먼저 검증하고 분석도구 명령은 실행하지 않습니다. 사람 검토, 검토 완료 실행, 증거 인증은 각각 별도 승인 단계를 유지합니다.'),
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '운영 산출물 전체 닫기는 기존 운영 분석도구 폴더를 묶음으로 만들고 가져오기, 결과 회수, close-e2e까지 이어서 수행하지만 분석도구, 컨테이너, 보조 실행 환경, 네트워크 스캔 명령은 실행하지 않습니다.'),
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '운영 산출물 묶음은 파일 위치와 해시를 확인한 뒤 가져옵니다. 도구 명령·능동 스캔은 실행하지 않고 검증된 파일만 toolchain collection으로 연결합니다.'),
            ...
            h('div', { style:{ display:'flex', gap:'8px', flexWrap:'wrap', alignItems:'center' } },
              ...
              }, operatingClosureReviewState.status === 'recording' ? '사람 검토 기록 중' : '운영 closure 사람 검토 기록'),
              h('button', {
              ...
              }, reviewedOperatingCloseState.status === 'reviewed-closing' ? '검토 완료 closure 실행 중' : '검토 완료 운영 closure 실행'),
              h('button', {
              ...
              }, reviewedCloseCertificationState.status === 'certifying' ? '증거 인증 중' : '운영 closure 증거 인증'),
              h('button', {
            ...
            this.renderTable(['운영 closure 다음 단계','상태','사용자 조치','연결'], operatingClosureReadinessStepRows.length ? operatingClosureReadinessStepRows : [['대기','-','운영 closure 준비 요약 버튼을 누르세요','연결 대기']]),
            this.renderTable(['운영 closure 준비 차단 조건','조치','다음 연결'], operatingClosureReadinessBlockerRows.length ? operatingClosureReadinessBlockerRows : [['없음','현재 표시된 준비 차단 조건 없음','-']]),
            ...
            this.renderTable(['실제 운영 증거 차단 조건','조치','경고'], realOperatingEvidenceBlockerRows.length ? realOperatingEvidenceBlockerRows : [['없음','현재 표시된 차단 조건 없음','-']]),
            this.renderTable(['운영 증거 제출 묶음 항목','상태','파일/해시/검토'], operatorEvidenceSubmissionManifestRows.length ? operatorEvidenceSubmissionManifestRows : [['대기','-','운영 증거 제출 묶음 초안 버튼을 누르세요']]),
            this.renderTable(['운영 증거 제출 묶음 누락','항목','조치'], operatorEvidenceSubmissionManifestBlockerRows.length ? operatorEvidenceSubmissionManifestBlockerRows : [['없음','현재 표시된 누락 없음','-']]),
            ...
            this.renderTable(['운영 closure 서명','상태','서명자'], operatingClosureReviewApproverRows.length ? operatingClosureReviewApproverRows : [['대기','-','승인자 서명 기록 필요']]),
            this.renderTable(['운영 closure 증거 검사','상태','근거'], reviewedCloseCertificationRows.length ? reviewedCloseCertificationRows : [['대기','-','검토 완료 운영 closure 실행 뒤 증거 인증 버튼을 누르세요']]),
            ...
            this.renderTable(['운영 completion audit 차단 조건','조치','경고'], operatingCompletionAuditBlockerRows.length ? operatingCompletionAuditBlockerRows : [['없음','현재 표시된 차단 조건 없음','-']]),
            ...
            this.renderTable(['전체 목표 완료 차단 조건','조치','남은 수'], goalCompletionBlockerRows.length ? goalCompletionBlockerRows : [['없음','현재 표시된 차단 조건 없음','-']]),
