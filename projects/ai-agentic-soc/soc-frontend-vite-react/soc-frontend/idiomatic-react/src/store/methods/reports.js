...
,
  malaxWorkflowPanel() {
    ...
    return h('div', { style:{ display:'flex', flexDirection:'column', gap:'14px' } },
      ...
      findingRows.length ? smallPanel('발견사항 후보', this.renderTable(['ID','제목','상태','신뢰도','관찰','분석가 검토','근거'], findingRows)) : null,
      evidenceRows.length ? smallPanel('증거 연결표', this.renderTable(['증거 ID','종류','신뢰도','구분','요약'], evidenceRows)) : null,
      visualRows.length ? smallPanel('시각 증거', this.renderTable(['캡처 ID','모드','상태','산출물','판정 아님'], visualRows)) : null,
      releaseRows.length ? smallPanel('최종 승인 차단 조건', this.renderTable(['유형','설명','담당'], releaseRows)) : null,
...
,
  redTeam2AnalysisDraft() {
    ...
    return {
      ...
      compositeArtifactManifestSourceDir:'',
      ...
      credentialScopes:'보고서 읽기, 스캔 상태 읽기',
...
,
  async createRedTeam2ToolExecutionPlan(action = null) {
    ...
    try {
      ...
      this.toast(`실행 계획: ${plan.status}`, plan.status === 'approval_required' ? 'warn' : 'success');
      this.logAudit('현재 분석가', `레드팀 분석2 실행 계획: ${selectedAction.action_id} · ${plan.status}`);
    } catch (err) {
...
,
  async importRedTeam2ScannerServiceReport() {
    ...
    if (!endpointUrl) {
      this.toast('서비스 결과 가져오기용 읽기 전용 접속 주소를 입력하세요', 'warn');
      return;
    }
...
,
  async attestRedTeam2SafeSmokeInstallCandidate() {
    ...
    const candidates = (toolchainRun.install_version_evidence_candidates || []).filter(item => item.status === 'candidate_ready');
    ...
    try {
      const res = await fetch('http://127.0.0.1:8765/api/redteam/v2/tool-install-version-evidence/attest-safe-smoke-candidates', {
        method:'POST',
        headers:{ 'Content-Type':'application/json' },
        body:JSON.stringify({
          case_id:caseId,
          ...
          review_note:'선택된 version-only 출력과 산출물 해시를 확인했고 설치 증거로만 기록합니다. 실행 승인이나 Finding 근거로 확정하지 않습니다.',
          operator_attests_output_matches_artifact:true,
          candidates,
        }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok || data.status === 'partial_or_invalid') throw new Error((data.errors || []).join(', ') || data.detail || `HTTP ${res.status}`);
      ...
      this.toast(`설치 확인 후보 ${data.recorded_count || 0}개를 운영자 검토 증거로 기록했습니다`, 'success');
      this.logAudit('레드팀 리드', `레드팀 분석2 설치 확인 후보 일괄 증거 기록: ${data.recorded_count || 0}개 · ${data.batch_id || '-'}`);
      await this.loadRedTeam2AnalysisStatus();
    } catch (err) {
...
,
  async buildRedTeam2OperatorEvidenceSubmissionManifestDraft() {
    ...
    const reportId = String(draft.reportId || 'RTA-2026-0301').trim();
    const target = String(draft.target || '').trim();
    const caseId = this.redTeamOperationCaseId(reportId, target || 'redteam2-operator-evidence-submission');
    let attachments = [];
    try {
      const raw = String(draft.operatorEvidenceSubmissionAttachmentsJson || '').trim();
      attachments = raw ? JSON.parse(raw) : [];
    } catch (err) {
      this.toast('운영 증거 제출 첨부 JSON 형식을 확인하세요: ' + (err?.message || String(err)), 'warn');
      return;
    }
    const payload = {
      case_id:caseId,
    ...
    this.setState(s => ({ redteam2OperatorEvidenceSubmissionManifestState:{ ...(s.redteam2OperatorEvidenceSubmissionManifestState || {}), status:'drafting', error:null } }));
    try {
      const res = await fetch('http://127.0.0.1:8765/api/redteam/v2/toolchains/operator-evidence-submission-manifest-draft', {
        method:'POST',
        headers:{ 'Content-Type':'application/json' },
        body:JSON.stringify(payload),
      });
      ...
      if (!res.ok) throw new Error(data.detail || `HTTP ${res.status}`);
      this.setState(s => ({
        redteam2OperatorEvidenceSubmissionManifestState:{ ...(s.redteam2OperatorEvidenceSubmissionManifestState || {}), status:data.status || 'submission_manifest_draft_blocked', result:data, checkedAt:new Date().toISOString(), error:null },
      }));
      this.toast(data.ready_for_submission_validation ? '운영 증거 제출 manifest 초안 준비' : '운영 증거 제출 manifest 보완 필요', data.ready_for_submission_validation ? 'success' : 'warn');
      this.logAudit('현재 분석가', `레드팀 분석2 운영 증거 제출 manifest 초안: ${data.manifest_id} · ${data.status}`);
    } catch (err) {
      this.setState(s => ({ redteam2OperatorEvidenceSubmissionManifestState:{ ...(s.redteam2OperatorEvidenceSubmissionManifestState || {}), status:'error', error:err?.message || String(err), checkedAt:new Date().toISOString() } }));
      this.toast('운영 증거 제출 manifest 초안 실패: ' + (err?.message || String(err)), 'warn');
    }
  }
,
...
,
  async recordRedTeam2OperatingClosureHumanReview() {
    const draft = this.redTeam2AnalysisDraft();
    const reportId = String(draft.reportId || 'RTA-2026-0301').trim();
    const target = String(draft.target || '').trim();
    const caseId = this.redTeamOperationCaseId(reportId, target || 'redteam2-operating-close');
    const pkg = this.state.redteam2OperatingClosurePackageState?.result || {};
    if (!pkg.package_id) {
      this.toast('먼저 운영 closure 제출 패키지를 확인하세요', 'warn');
      return;
    }
    const payload = {
      case_id:caseId,
      package_id:pkg.package_id,
      reviewed_by:String(draft.compositeClosureReviewer || pkg.close_api_payload?.reviewed_by || 'lead@example.com').trim(),
      runtime_blocker_disposition:(pkg.runtime_blockers || []).length ? 'accepted' : 'none',
      ...
      approver_signoffs:{
        reviewed_by:String(draft.compositeClosureReviewer || pkg.close_api_payload?.reviewed_by || 'lead@example.com').trim(),
        lead_approver:String(draft.compositeClosureLead || pkg.close_api_payload?.lead_approver || 'lead@example.com').trim(),
        business_owner_approver:String(draft.compositeClosureBusinessOwner || pkg.close_api_payload?.business_owner_approver || 'business-owner@example.com').trim(),
        export_approver:String(draft.compositeClosureExportApprover || pkg.close_api_payload?.export_approver || 'executive-sponsor@example.com').trim(),
      },
    };
    this.setState(s => ({ redteam2OperatingClosureReviewState:{ ...(s.redteam2OperatingClosureReviewState || {}), status:'recording', error:null } }));
    try {
      const res = await fetch('http://127.0.0.1:8765/api/redteam/v2/toolchains/operating-closure-human-review', {
        method:'POST',
        headers:{ 'Content-Type':'application/json' },
        body:JSON.stringify(payload),
      });
      ...
      if (!res.ok) throw new Error(data.detail || `HTTP ${res.status}`);
      this.setState(s => ({
        redteam2OperatingClosureReviewState:{ ...(s.redteam2OperatingClosureReviewState || {}), status:data.status || 'review_required', result:data, checkedAt:new Date().toISOString(), error:null },
      }));
      this.toast(data.ready_for_human_close_execution ? '운영 closure 사람 검토 기록 완료' : '운영 closure 사람 검토 보완 필요', data.ready_for_human_close_execution ? 'success' : 'warn');
      this.logAudit('현재 분석가', `레드팀 분석2 운영 closure 사람 검토: ${data.package_id} · ${data.status}`);
    } catch (err) {
      this.setState(s => ({ redteam2OperatingClosureReviewState:{ ...(s.redteam2OperatingClosureReviewState || {}), status:'error', error:err?.message || String(err), checkedAt:new Date().toISOString() } }));
      this.toast('운영 closure 사람 검토 기록 실패: ' + (err?.message || String(err)), 'warn');
    }
  }
,
  async executeRedTeam2ReviewedOperatingClose() {
    const draft = this.redTeam2AnalysisDraft();
    const reportId = String(draft.reportId || 'RTA-2026-0301').trim();
    const target = String(draft.target || '').trim();
    const caseId = this.redTeamOperationCaseId(reportId, target || 'redteam2-operating-close');
    const review = this.state.redteam2OperatingClosureReviewState?.result || {};
    if (!review.review_id) {
      this.toast('먼저 운영 closure 사람 검토 기록을 완료하세요', 'warn');
      return;
    }
    const payload = {
      case_id:caseId,
    ...
    this.setState(s => ({ redteam2ReviewedOperatingCloseState:{ ...(s.redteam2ReviewedOperatingCloseState || {}), status:'reviewed-closing', error:null } }));
    try {
      const res = await fetch('http://127.0.0.1:8765/api/redteam/v2/toolchains/execute-reviewed-operating-close', {
        method:'POST',
        headers:{ 'Content-Type':'application/json' },
        body:JSON.stringify(payload),
      });
      ...
      this.setState(s => ({
        redteam2ReviewedOperatingCloseState:{ ...(s.redteam2ReviewedOperatingCloseState || {}), status:data.status || 'blocked', result:data, checkedAt:new Date().toISOString(), error:null },
        redteam2ToolchainClosureState:data.close_result ? { ...(s.redteam2ToolchainClosureState || {}), status:data.close_result.status, result:data.close_result, checkedAt:new Date().toISOString(), error:null } : (s.redteam2ToolchainClosureState || {}),
      }));
      this.toast(data.complete ? '검토 완료 운영 closure 실행 완료' : '검토 완료 운영 closure 실행 차단', data.complete ? 'success' : 'warn');
      this.logAudit('현재 분석가', `레드팀 분석2 검토 완료 운영 closure 실행: ${data.review_id} · ${data.status}`);
    } catch (err) {
      this.setState(s => ({ redteam2ReviewedOperatingCloseState:{ ...(s.redteam2ReviewedOperatingCloseState || {}), status:'error', error:err?.message || String(err), checkedAt:new Date().toISOString() } }));
      this.toast('검토 완료 운영 closure 실행 실패: ' + (err?.message || String(err)), 'warn');
    }
  }
,
  async certifyRedTeam2ReviewedOperatingCloseEvidence() {
    const draft = this.redTeam2AnalysisDraft();
    const reportId = String(draft.reportId || 'RTA-2026-0301').trim();
    const target = String(draft.target || '').trim();
    const caseId = this.redTeamOperationCaseId(reportId, target || 'redteam2-operating-close');
    const execution = this.state.redteam2ReviewedOperatingCloseState?.result || {};
    if (!execution.execution_id) {
      this.toast('먼저 검토 완료 운영 closure 실행을 완료하세요', 'warn');
      return;
    }
    const payload = {
      case_id:caseId,
      execution_id:execution.execution_id,
      certified_by:String(draft.compositeClosureReviewer || 'lead@example.com').trim(),
    ...
    this.setState(s => ({ redteam2ReviewedCloseCertificationState:{ ...(s.redteam2ReviewedCloseCertificationState || {}), status:'certifying', error:null } }));
    try {
      const res = await fetch('http://127.0.0.1:8765/api/redteam/v2/toolchains/certify-reviewed-operating-close-evidence', {
        method:'POST',
        headers:{ 'Content-Type':'application/json' },
        body:JSON.stringify(payload),
      });
      ...
      if (!res.ok) throw new Error(data.detail || `HTTP ${res.status}`);
      this.setState(s => ({
        redteam2ReviewedCloseCertificationState:{ ...(s.redteam2ReviewedCloseCertificationState || {}), status:data.status || 'certification_required', result:data, checkedAt:new Date().toISOString(), error:null },
      }));
      this.toast(data.ready_for_completion_audit_review ? '운영 closure 증거 인증 준비 완료' : '운영 closure 증거 인증 보완 필요', data.ready_for_completion_audit_review ? 'success' : 'warn');
      this.logAudit('현재 분석가', `레드팀 분석2 운영 closure 증거 인증: ${data.execution_id} · ${data.status}`);
    } catch (err) {
      this.setState(s => ({ redteam2ReviewedCloseCertificationState:{ ...(s.redteam2ReviewedCloseCertificationState || {}), status:'error', error:err?.message || String(err), checkedAt:new Date().toISOString() } }));
      this.toast('운영 closure 증거 인증 실패: ' + (err?.message || String(err)), 'warn');
    }
  }
,
  async reviewRedTeam2OperatingCompletionAuditCandidate() {
    const draft = this.redTeam2AnalysisDraft();
    const reportId = String(draft.reportId || 'RTA-2026-0301').trim();
    const target = String(draft.target || '').trim();
    const caseId = this.redTeamOperationCaseId(reportId, target || 'redteam2-operating-completion-audit');
    const certification = this.state.redteam2ReviewedCloseCertificationState?.result || {};
    if (!certification.certification_id) {
      this.toast('먼저 운영 closure 증거 인증을 완료하세요', 'warn');
      return;
    }
    const payload = {
      case_id:caseId,
      certification_id:certification.certification_id,
      audited_by:String(draft.compositeClosureReviewer || 'independent-auditor@example.com').trim(),
    };
    this.setState(s => ({ redteam2OperatingCompletionAuditState:{ ...(s.redteam2OperatingCompletionAuditState || {}), status:'auditing', error:null } }));
    try {
      const res = await fetch('http://127.0.0.1:8765/api/redteam/v2/toolchains/review-operating-completion-audit-candidate', {
        method:'POST',
        headers:{ 'Content-Type':'application/json' },
        body:JSON.stringify(payload),
      });
      ...
      if (!res.ok) throw new Error(data.detail || `HTTP ${res.status}`);
      this.setState(s => ({
        redteam2OperatingCompletionAuditState:{ ...(s.redteam2OperatingCompletionAuditState || {}), status:data.status || 'completion_audit_blocked', result:data, checkedAt:new Date().toISOString(), error:null },
      }));
      this.toast(data.goal_complete_candidate ? '완료 감사 후보 준비 완료' : '완료 감사 차단 항목 확인 필요', data.goal_complete_candidate ? 'success' : 'warn');
      this.logAudit('현재 분석가', `레드팀 분석2 완료 감사 검토: ${data.certification_id} · ${data.status}`);
    } catch (err) {
      this.setState(s => ({ redteam2OperatingCompletionAuditState:{ ...(s.redteam2OperatingCompletionAuditState || {}), status:'error', error:err?.message || String(err), checkedAt:new Date().toISOString() } }));
      this.toast('완료 감사 검토 실패: ' + (err?.message || String(err)), 'warn');
    }
  }
,
...
,
  redTeamAnalysis2Panel() {
    ...
    const showAdminDetails = this.state.redteam2ShowAdminDetails === true;
    ...
    const statusLabelMap = {
      ...
      recording:'사람 검토 기록 중',
      'reviewed-closing':'검토 완료 closure 실행 중',
      reviewed_operating_close_complete:'검토 완료 운영 closure 완료',
      certifying:'증거 인증 중',
      certification_required:'증거 인증 보완 필요',
      ready_for_completion_audit_review:'완료 감사 검토 준비',
      auditing:'완료 감사 검토 중',
      ...
      invalid:'무효',
      valid:'유효',
      install_required:'설치 확인 필요',
      registered_verified:'등록 확인됨',
      tool_execution_waiting_on_environment:'환경 준비 대기',
      tool_action_card_required:'승인 작업 카드 필요',
    ...
    const koStored = value => value ? '분석 저장소에 보관됨' : '아직 보관되지 않음';
    const koHiddenLocation = value => value ? '세부 위치는 관리자/감사 기록에서 확인' : '보관 위치 없음';
    const koReadyLink = value => value ? '연결 준비됨' : '연결 대기';
    const koCountSummary = (values, empty = '없음') => Array.isArray(values) && values.length ? `${values.length}개 항목` : empty;
    const koBlockerSummary = (values, empty = '차단 조건 없음') => Array.isArray(values) && values.length ? `${values.length}개 차단 조건` : empty;
    const koToolName = value => ({
      'TOOL-NUCLEI-001':'Nuclei',
      'TOOL-OPENVAS-001':'OpenVAS',
      'TOOL-TRIVY-001':'Trivy',
      'TOOL-SCA-001':'SCA',
      'TOOL-NPM-AUDIT-001':'npm audit',
      'TOOL-ZAP-001':'OWASP ZAP',
    }[String(value || '')] || String(value || '-'));
    const koToolDisplayName = (displayName, toolId) => {
      const raw = String(displayName || '').trim();
      if (raw && /^TOOL-[A-Z0-9-]+$/.test(raw)) return koToolName(raw);
      return raw || koToolName(toolId);
    };
    const koApiLink = value => {
      const raw = String(value || '').trim();
      if (!raw) return '-';
      const compact = raw.replace(/^https?:\/\/127\.0\.0\.1:\d+\//, '/').replace(/^\/+/, '');
      const known = [
        ['six-tool-work-order', '작업 순서 연결'],
        ['six-tool-submission-template', '제출 양식 연결'],
        ['operator-evidence-submission-manifest-draft', '증거 제출 검증 연결'],
        ['collect-results', '결과 회수 연결'],
        ['operating-closure-readiness-summary', '운영 검토 준비 연결'],
        ['run-status', '저장 상태 조회 연결'],
        ['launch-readiness', '실행 준비 확인 연결'],
        ['scanner-service-imports', '읽기 전용 결과 가져오기 연결'],
      ];
      const label = (known.find(([needle]) => compact.includes(needle)) || [null, '연결 준비됨'])[1];
      return showAdminDetails ? `${label} · ${raw}` : label;
    };
    const koEvidenceTerm = value => String(value || '')
      .replace(/nuclei_result_normalizer_agent/g, '도구별 분석 에이전트')
      .replace(/openvas_result_normalizer_agent/g, '도구별 분석 에이전트')
      .replace(/trivy_result_normalizer_agent/g, '도구별 분석 에이전트')
      .replace(/sca_result_normalizer_agent/g, '도구별 분석 에이전트')
      .replace(/npm_audit_result_normalizer_agent/g, '도구별 분석 에이전트')
      .replace(/zap_result_normalizer_agent/g, '도구별 분석 에이전트')
      .replace(/normalizer/g, '결과 정리기')
      .replace(/Sanitizer/g, '안전 정리기')
      .replace(/sanitizer/g, '안전 정리기')
      .replace(/manifest/g, '제출 묶음')
      .replace(/tool_action_card_required/g, '승인 작업 카드 필요')
      .replace(/tool_action:approve_t3/g, '고위험 승인 필요')
      .replace(/evidence:approve/g, '증거 승인')
      .replace(/finding:approve_severity/g, '심각도 승인')
      .replace(/report:approve_export/g, '보고서 내보내기 승인')
      .replace(/collect-results/g, '결과 회수')
      .replace(/manual_operator_run/g, '사람이 실행하고 결과 업로드')
      .replace(/ToolActionCard/g, '승인 작업 카드')
      .replace(/Claim-Evidence Matrix/g, '주장-증거 연결표')
      .replace(/Evidence Card/g, '증거 카드')
      .replace(/\bEvidence\b/g, '증거')
      .replace(/Report v2/g, '한국어 보고서 v2')
      .replace(/Agentic RAG/g, '근거 검색 검증')
      .replace(/Matrix/g, '연결표');
    const koAgentName = value => {
      const raw = String(value || '').trim();
      if (!raw) return '-';
      return /[_-]agent$|normalizer|reasoning/i.test(raw) ? '도구별 분석 에이전트' : raw;
    };
    const queueCards = queue.map(action => {
      ...
      h('div', { style:{ minWidth:0 } },
        h('div', { style:{ fontSize:'11px', fontWeight:900, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' } }, showAdminDetails ? (action.action_id || '-') : `승인 작업 · ${koToolName(action.tool_id)}`),
        h('div', { style:{ fontSize:'9.5px', color:C.sec, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' } }, koEvidenceTerm(action.title || action.objective || '-')),
    ...
    const gateRows = st.lastAction ? [
      ...
      ['증거', '증거 후보 필요', '도구 출력은 명령이 아니라 데이터로만 취급'],
    ...
    const toolRows = analysisTools.map(tool => [
      koToolDisplayName(tool.display_name || tool.name, tool.tool_id),
      koRiskClass(tool.risk_class),
      koValue(tool.runtime_status),
      koAgentName(tool.llm_agent?.name || tool.agent_id),
    ]);
    ...
    const selectedInstallRows = [
      ...
      ['설치 방식', koCountSummary(selectedInstall.install_modes || [], '설치 방식 미확인'), selectedInstall.official_url ? '공식 출처 등록됨' : '공식 출처 미등록'],
      ['사람이 확인할 명령', (selectedInstall.operator_install_commands || []).length ? `${selectedInstall.operator_install_commands.length}개 확인 명령` : '-', 'API가 직접 실행하지 않음'],
      ['검증 방법', (selectedInstall.verification_commands || []).length ? `${selectedInstall.verification_commands.length}개 검증 방법` : '-', selectedInstall.commands_executed_by_api === false ? '사람이 실행한 증거 필요' : '미확인'],
      ['증거 연결', selectedInstall.evidence_pipeline ? '도구별 분석 연결됨' : '-', selectedInstall.evidence_pipeline?.analysis_agent_id ? 'LLM 분석 에이전트 확인' : '-'],
    ];
    const credentialPolicyRows = [
      ...
      ['허용 범위', (selectedCredentialPolicy.allowed_token_scopes || []).length ? `${selectedCredentialPolicy.allowed_token_scopes.length}개 읽기 전용 범위` : '-', '읽기 전용 범위만 허용'],
      ['금지 범위', (selectedCredentialPolicy.prohibited_token_scopes || []).length ? `${selectedCredentialPolicy.prohibited_token_scopes.length}개 금지 범위` : '-', '능동 스캔, 관리자, 쓰기 계열 차단'],
      ['비밀값 저장', koBool(selectedCredentialPolicy.secret_material_stored ?? false), '접속 비밀값은 저장하지 않음'],
    ...
    const credentialAuthorizationRows = [
      ...
      ['외부 비밀 보관 참조', selectedCredentialAuthorization.credential_ref || draft.credentialRef ? '참조 등록됨' : '-', '비밀값이 아니라 참조만 저장'],
      ['읽기 전용 접속 참조', selectedCredentialAuthorization.endpoint_ref || draft.credentialEndpointRef ? '참조 등록됨' : '-', '승인된 읽기 전용 접속만 사용'],
      ['범위', (selectedCredentialAuthorization.token_scopes || []).length ? `${selectedCredentialAuthorization.token_scopes.length}개 범위` : (draft.credentialScopes || '-'), '허용목록과 비교됨'],
      ['명령 실행', koBool(selectedCredentialAuthorization.commands_executed_by_api ?? false), '권한 승인 기능은 도구를 실행하지 않음'],
    ...
    const serviceImportRows = [
      ...
      ['읽기 전용 접속', serviceImportResult.endpoint_url || draft.serviceImportEndpointUrl || selectedCredentialAuthorization.endpoint_ref ? '참조 확인됨' : '-', '승인된 읽기 전용 서비스 URL'],
      ['능동 스캔 실행', koBool(serviceImportResult.active_scan_executed ?? false), '항상 아니오여야 함'],
      ['비밀값 저장', koBool(serviceImportResult.secret_material_stored ?? false), '접속 비밀값 저장 금지'],
      ['명령으로 신뢰 여부', koBool(serviceImportResult.trusted_as_instruction ?? false), '항상 아니오 유지'],
      ['증거', serviceImportEvidence.evidence_id ? '증거 후보 준비됨' : '-', koValue(serviceImportEvidence.approval_status || '증거 후보')],
    ...
    const serviceImportAnalystProgressRows = [
      ...
      ['회수 가능 결과', `${serviceImportAnalystProgress.collectable_count ?? (serviceImportResult.tool_run?.run_id ? 1 : 0)}개`, '결과 회수·증거 후보 단계로 넘길 수 있는 서비스 가져오기 결과'],
      ['증거 후보', `${serviceImportAnalystProgress.evidence_candidate_count ?? 0}개`, '승인 전에는 Finding이나 보고서 주장으로 확정하지 않음'],
    ];
    const serviceImportAnalystStageRows = (serviceImportAnalystProgress.stage_rows || []).map(item => [
      item.title_ko || '진행 단계',
      koValue(item.status || '대기'),
      item.button_ko || '-',
      koEvidenceTerm(item.summary_ko || item.next_action_ko || '-'),
    ]);
    ...
    const launchReadinessRows = launchButtons.map(item => [
      koToolDisplayName(item.display_name, item.tool_id),
      ...
      koApiLink(item.primary_api),
    ]);
    const sixToolWorkOrderRows = (sixToolWorkOrder.work_order_rows || []).map(item => [
      `${item.index || '-'} · ${koToolDisplayName(item.display_name, item.tool_id)}`,
    ...
    const sixToolSubmissionTemplateRows = (sixToolSubmissionTemplate.collection_package?.collection_items || []).map(item => [
      item.title_ko || koToolName(item.tool_id),
    ...
    const analystExecutionGuideRows = [
      ['1', '6개 도구 작업 순서 만들기', '도구마다 다음에 누를 버튼을 확인합니다', koApiLink('/api/redteam/v2/toolchains/six-tool-work-order')],
      ['2', '6개 도구 제출 양식 만들기', '실제 결과 파일 정보를 채울 양식을 만듭니다', koApiLink('/api/redteam/v2/toolchains/six-tool-submission-template')],
      ['3', '결과 회수·증거 후보', '도구 결과를 증거 후보와 분석 브리프로 정리합니다', koApiLink('/api/redteam/v2/toolchains/{toolchain_id}/collect-results')],
    ];
    const analystReadinessRows = [
      ['현재 안내', koValue(analystReadinessSummary.status || '대기'), koEvidenceTerm(analystReadinessSummary.message_ko || '분석가용 안내를 불러오려면 상태 새로고침을 누르세요').replace(/Docker, WSL, OpenVAS\/ZAP endpoint 같은 환경 준비는 관리자용 항목에서 처리합니다\./g, '관리자 준비 항목은 별도 설정에서 처리합니다.')],
      ['다음 버튼', koEvidenceTerm(analystReadinessSummary.primary_next_button_ko || '여러 도구 결과 첨부'), koEvidenceTerm(analystReadinessSummary.secondary_next_button_ko || '결과 회수·증거 후보')],
      ['결과 첨부', analystReadinessSummary.can_attach_operator_results === false ? '불가' : '가능', '운영자가 실행한 Nuclei/OpenVAS/Trivy/SCA/npm audit/ZAP 결과 파일을 증거 후보로 연결'],
    ...
    const analystRoleNextStepRows = (analystReadinessSummary.next_steps || []).map(item => [
      koEvidenceTerm(item.button_ko || item.title_ko || '-'),
      koValue(item.status || '미확인'),
      koEvidenceTerm(item.message_ko || '-').replace(/Docker|WSL|OpenVAS\/ZAP endpoint/g, '관리자 준비 항목'),
      koApiLink(item.api),
    ]);
    const analystPlainBlockerRows = (analystReadinessSummary.plain_language_blockers_ko || []).map((item, index) => [
      `${index + 1}`,
      koEvidenceTerm(item).replace(/Docker|WSL|OpenVAS\/ZAP/g, '관리자 준비 항목'),
      '관리자 준비 확인 또는 결과 첨부로 진행',
    ]);
    ...
    const runtimeReadinessRows = [
      ['전체 상태', koValue(operatorEnvironmentSummary.status || runtimeReadiness.status || '미확인'), operatorEnvironmentSummary.message_ko || koBlockerSummary(runtimeReadiness.blockers, '실측 조건 차단 없음')],
      ['도구 실행 가능 여부', runtimeReadiness.tool_execution_ready ? '가능' : '차단됨', koBlockerSummary(runtimeReadiness.tool_execution_blocked_by, '실행 차단 단계 없음')],
      ...
      ['컨테이너 실행 환경', containerRuntime.runtime_preflight?.ready ? '준비됨' : '차단됨', containerRuntime.runtime_preflight?.blocker || koValue(containerRuntime.status) || '상태 확인 필요'],
      ['컨테이너 점검 산출물', koValue(containerRuntime.status || containerRuntimeArtifact.status || '미확인'), koStored(containerRuntime.artifact_path || containerRuntimeArtifact.path)],
      ['보조 실행 환경', koValue(wslRuntime.status || wslRuntimeArtifact.status || '미확인'), koBlockerSummary(wslRuntime.blockers, wslRuntime.selected_distro ? '배포판 확인됨' : '보조 실행 환경 증거 필요')],
      ['보조 실행 배포판', wslRuntime.selected_distro || '-', koCountSummary(wslRuntime.distros, '목록 확인 전')],
      ['실측 승격 게이트', koValue(strictPromotion.status || strictPromotionArtifact.status || '미확인'), koBlockerSummary(strictPromotion.blockers, '컨테이너/보조 실행/읽기 전용 도구 게이트 통과 필요')],
      ...
      ['조치 안내서', koValue(liveRemediation.status || liveRemediationArtifact.status || '미확인'), koHiddenLocation(liveRemediation.markdown_artifact_path || liveRemediationArtifact.path)],
      ['남은 조치 단계', `${liveRemediation.blocked_step_count ?? '-'}개`, `${liveRemediation.step_count ?? 5}개 단계 중 운영자 조치 필요`],
      ['증거 수집 패키지', koValue(operatorEvidence.status || operatorEvidenceArtifact.status || '미확인'), koHiddenLocation(operatorEvidence.markdown_artifact_path || operatorEvidenceArtifact.path)],
      ['수집할 증거 항목', `${operatorEvidence.blocked_collection_item_count ?? '-'}개`, `${operatorEvidence.collection_item_count ?? 5}개 항목 중 운영자 증거 필요`],
      ['증거 제출 검증', koValue(operatorSubmission.status || operatorSubmissionArtifact.status || '미확인'), koHiddenLocation(operatorSubmission.markdown_artifact_path || operatorSubmissionArtifact.path)],
      ...
      ['Evidence Card 후보 계획', koValue(operatorImportPlan.status || operatorImportPlanArtifact.status || '미확인'), koHiddenLocation(operatorImportPlan.markdown_artifact_path || operatorImportPlanArtifact.path)],
      ['Evidence Card 후보 수', `${operatorImportPlan.candidate_count ?? 0}개`, `${operatorImportPlan.blocked_item_count ?? '-'}개 항목이 아직 차단됨`],
      ['도구 결과 LLM 분석 브리프', koValue(toolResultAnalysis.status || toolResultAnalysisArtifact.status || '미확인'), koHiddenLocation(toolResultAnalysisArtifact.path)],
      ...
      ['Finding/Claim 검토 패키지', koValue(findingClaimReview.status || findingClaimReviewArtifact.status || '미확인'), koHiddenLocation(findingClaimReviewArtifact.path)],
      ...
      ['복합 도구 결과 회수', koReadyLink(true), '저장된 결과만 읽어 정규화와 Evidence Card 후보 생성을 수행합니다. 승인 전에는 Finding이나 보고서 Claim으로 확정하지 않습니다'],
      ['운영 산출물 묶음 만들기', koReadyLink(true), '운영 산출물 폴더에서 결과 파일을 찾아 해시 묶음을 만들고 명령은 실행하지 않습니다'],
      ['운영 산출물 가져오기', koReadyLink(true), '파일 위치와 해시를 검증해 6개 분석도구 결과를 한 collection으로 가져옵니다'],
      ['실제 운영 증거 사전 점검', koReadyLink(true), '테스트 경로나 샘플 산출물을 운영 closure 전에 차단합니다'],
      ['운영 증거 제출 묶음 초안', koReadyLink(true), '운영자가 첨부한 파일 해시와 상태를 확인하고 사람 승인 전까지 완료로 보지 않습니다'],
      ['운영 Evidence Card 후보 가져오기', koReadyLink(true), '승인된 운영자 제출 증거 후보를 Evidence Card로 등록하고 명시적 사람 검토가 있을 때만 승인 기록을 남깁니다'],
      ['운영 closure 제출 패키지', koReadyLink(true), '운영 산출물 폴더, 승인자 4명, 실행 차단 조건, 닫기 요청을 실행 전 검증합니다'],
      ['운영 closure 사람 검토', koReadyLink(true), '제출 패키지 체크리스트, 승인자 서명, 차단 조건 처리 방침을 실행 전 기록합니다'],
      ['검토 완료 운영 closure 실행', koReadyLink(true), '사람 검토 기록의 승인된 닫기 요청만 사용해 별도 HITL close를 실행합니다'],
      ['운영 closure 증거 인증', koReadyLink(true), '실제 운영 산출물과 실제 승인자 attestation을 completion audit 후보로 검증합니다'],
      ['운영 completion audit 검토', koReadyLink(true), '인증 후보를 독립 감사 checklist로 다시 검토하고 테스트 산출물은 완료 후보에서 차단합니다'],
      ['복합 Evidence 후보 승인', koReadyLink(true), '레드팀 리드 또는 통제팀이 후보 Evidence를 승인해야 Finding 승격과 Matrix 준비로 이동합니다'],
      ['복합 Finding 초안 생성', koReadyLink(true), '승인된 Evidence만 pending review Finding 초안으로 만들며 2인 승인 전 보고서 Claim 반영은 보류합니다'],
      ['복합 Finding 심각도 2인 승인', koReadyLink(true), '초안 Finding만 레드팀 리드와 업무 소유자가 함께 승인하며 Matrix와 보고서 검증은 다음 단계로 남깁니다'],
      ['복합 Collection Matrix 초안', koReadyLink(true), '승인된 Evidence와 2인 승인 Finding만 ready row로 구성하며 보류 row는 보고서 입력에서 제외합니다'],
      ['복합 Collection Report v2 draft', koReadyLink(true), 'Matrix ready와 report gate pass일 때만 한국어 Report v2 draft를 생성하고 최종 export 승인은 별도로 남깁니다'],
      ['복합 Collection E2E 완료 게이트', koReadyLink(true), 'Evidence 승인, Finding 승격, 2인 severity 승인, Matrix ready, Report gate, export 완료를 기존 산출물로만 점검합니다'],
      ['운영 산출물 전체 닫기', koReadyLink(true), '운영 결과 폴더를 묶음으로 만들고 가져오기, 결과 회수, close-e2e를 이어서 수행하되 분석도구 명령은 실행하지 않습니다'],
      ['Claim-Evidence Matrix 초안', koReadyLink(true), '승인된 Evidence와 2인 severity 승인된 Finding만 보고서 검증 입력에 포함합니다'],
      ['Matrix 기반 Report v2 draft', koReadyLink(true), '보류 row 0건과 report gate pass일 때만 한국어 Report v2 draft를 생성합니다'],
      ['OpenVAS 읽기 전용 연결', koValue(openvasReadiness.status || externalScanner.status || '미확인'), koBlockerSummary(openvasReadiness.blockers, openvasReadiness.endpoint_env ? '연결 참조 등록됨' : '읽기 전용 연결 참조 필요')],
      ['ZAP 읽기 전용 연결', koValue(zapReadiness.status || externalScanner.status || '미확인'), koBlockerSummary(zapReadiness.blockers, zapReadiness.endpoint_env ? '연결 참조 등록됨' : '읽기 전용 연결 참조 필요')],
      ['실서비스 가져오기', koValue(externalServiceImport.status || externalServiceImportArtifact.status || '미확인'), externalServiceImport.service_endpoint_fetch_executed ? '조직 읽기 전용 보고서 가져오기 수행됨' : '기본값은 가져오기 미실행, 승인 후 네트워크 허용 필요'],
      ...
      ['외부 비밀 보관 참조', externalScanner.vault_reference_ready ? '준비됨' : '설정 필요', '비밀값은 저장하지 않고 승인된 외부 보관 참조만 사용'],
      ['조회 기능의 명령 실행', koBool(runtimeReadiness.commands_executed_by_api ?? false), '상태 조회 기능은 컨테이너나 분석도구를 실행하지 않음'],
    ...
    const liveRemediationDefaultStepRows = [
      ['컨테이너 실행 환경 준비', '미확인', '컨테이너 점검 산출물 첨부'],
      ['보조 실행 환경 복구', '미확인', '보조 실행 환경 준비 증거 첨부'],
      ['OpenVAS/ZAP 읽기 전용 연결과 외부 비밀 보관 참조 설정', '미확인', '비밀값이 아닌 외부 보관 참조만 설정'],
      ['OpenVAS/ZAP 읽기 전용 보고서 가져오기 실측', '미확인', '승인된 읽기 전용 연결에서만 가져오기 산출물 첨부'],
      ['최종 실측 승격', '미확인', '모든 단계 통과 뒤 실측 승격 산출물 첨부'],
    ];
    const liveRemediationStepRows = (liveRemediation.steps || []).map(item => [
      ...
      [
        ...
        item.verification_command ? '확인 절차: 관리자 감사 기록에서 확인' : null,
      ].filter(Boolean).join(' · '),
    ]);
    const operatorEvidenceDefaultRows = [
      ['컨테이너 실행 환경 준비 증거', '미확인', '컨테이너 점검 산출물을 Evidence Card 후보로 첨부'],
      ['보조 실행 환경 증거', '미확인', '보조 실행 환경 준비 산출물을 Evidence Card 후보로 첨부'],
      ['OpenVAS/ZAP 읽기 전용 연결 증거', '미확인', '비밀값 없이 연결 참조와 외부 보관 참조 승인 증거 첨부'],
      ['읽기 전용 보고서 가져오기 증거', '미확인', 'OpenVAS/ZAP 읽기 전용 가져오기 산출물 첨부'],
      ['최종 실측 승격 증거', '미확인', '최종 실측 승격 산출물 첨부'],
    ];
    ...
    const operatorSubmissionDefaultRows = [
      ['증거 제출 묶음', '제출 대기', '보관 위치, 파일 해시, 사람 승인 상태가 필요'],
      ['산출물 상태 확인', '제출 대기', '각 산출물 상태가 기대 상태와 일치해야 함'],
      ['파일 해시 확인', '제출 대기', '제출 묶음의 파일 해시와 실제 파일 해시가 일치해야 함'],
    ...
    const operatorEvidenceSubmissionManifestRows = (operatorEvidenceSubmissionManifest.attachment_rows || []).map(item => [
      item.item_id || '-',
      koValue(item.status || '미확인'),
      [
        item.artifact_path ? '파일: 분석 저장소에 보관됨' : '파일 보관 위치 없음',
        item.sha256 ? `파일 해시: ${String(item.sha256).slice(0, 12)}...` : '파일 해시 없음',
        `상태: ${item.artifact_status || '-'} / 기대: ${item.expected_status || '-'}`,
        `사람 검토: ${koValue(item.review_status || 'pending_human_review')}`,
      ].filter(Boolean).join(' · '),
    ]);
    const operatorEvidenceSubmissionManifestBlockerRows = [
      ...(operatorEvidenceSubmissionManifest.errors || []).map(item => ['차단', item, '수정 후 다시 초안 생성']),
      ...(operatorEvidenceSubmissionManifest.missing_items || []).map(item => ['누락', item, '첨부 JSON에 항목 ID와 보관 위치 추가']),
    ...
    const operatorEvidenceCardImportRows = (operatorEvidenceCardImport.import_rows || []).map(item => [
      ...
      [
        ...
        item.evidence_artifact_path ? 'Evidence 후보: 분석 저장소에 보관됨' : null,
        (item.errors || []).length ? `오류: ${(item.errors || []).slice(0, 3).join(', ')}` : null,
      ].filter(Boolean).join(' · '),
    ]);
    ...
    const toolResultEvidenceRows = (toolResultAnalysis.evidence_pack || []).map(item => [
      ...
      item.evidence_id || '-',
      [
        item.result_id ? '결과 정리됨' : null,
        item.run_id ? '실행 기록 연결됨' : null,
        item.agent_id ? `에이전트: ${koAgentName(item.agent_id)}` : null,
      ].filter(Boolean).join(' · '),
    ]);
    ...
    const toolGuideProfiles = {
      'TOOL-NUCLEI-001': {
        ...
        beginnerNext:'먼저 승인 작업 계획을 만들고 승인 범위와 템플릿 목록을 확인한 뒤, 리드 승인을 받아 수동 실행 결과 파일을 업로드하세요.',
        safeMode:'권장 시작: 계획만 만들기 또는 이미 있는 결과 파일 분석. 실험망 실행은 승인된 실험망에서만 사용합니다.',
        evidence:'Nuclei 결과는 취약점 후보 증거로 변환되고, LLM 에이전트가 오탐/근거 부족 표현을 보류합니다.',
      },
      'TOOL-OPENVAS-001': {
        ...
        safeMode:'권장 시작: 보고서 가져오기 또는 사람이 실행하고 결과 업로드. 운영망 직접 스캔은 기본 차단입니다.',
      ...
      'TOOL-TRIVY-001': {
        ...
        safeMode:'권장 시작: 이미 있는 결과 파일 분석 또는 격리된 샌드박스 실행. 원격 저장소 접근은 별도 승인 전 금지합니다.',
      ...
      'TOOL-SCA-001': {
        ...
        beginnerNext:'잠금 파일, SBOM, 의존성 목록처럼 이미 존재하는 파일을 업로드하고 LLM 에이전트가 보고서 표현을 제한하게 하세요.',
        safeMode:'권장 시작: 이미 있는 결과 파일 분석. 패키지 다운로드나 사설 저장소 접근은 승인 전 금지합니다.',
        evidence:'SCA 결과는 컴포넌트 인벤토리 증거와 취약점 후보 증거로 나뉘며, 영향 범위 연결은 사람이 검토하기 전 주장으로 확정하지 않습니다.',
      },
      'TOOL-NPM-AUDIT-001': {
        summary:'Node.js lockfile 기반 의존성 취약점 점검입니다. 자동 수정은 하지 않고 JSON 결과만 증거화합니다.',
        beginnerNext:'승인된 잠금 파일이 있는 작업 폴더에서 사람이 npm audit 결과를 만들고 결과를 업로드하세요.',
        safeMode:'권장 시작: 이미 있는 결과 파일 분석. 자동 수정, 배포, 인증 저장소 접근은 금지합니다.',
        evidence:'npm audit 결과는 권고, 패키지, 심각도, 수정 가능 여부를 증거 후보로 만듭니다.',
      },
      'TOOL-ZAP-001': {
        summary:'웹 애플리케이션 점검 도구입니다. passive/import는 낮은 위험으로 다루고 active scan은 승인된 lab에서만 수행합니다.',
        beginnerNext:'먼저 수동 점검 결과 또는 기존 ZAP 보고서를 가져오고, 능동 스캔이 필요하면 승인 작업 뒤 실험망 실행으로 제한하세요.',
        safeMode:'권장 시작: 이미 있는 결과 파일 분석 또는 수동 보고서 가져오기. 공격 모드와 무제한 탐색은 금지합니다.',
    ...
    const selectedToolGuide = toolGuideProfiles[draft.analysisToolId] || {
      ...
      beginnerNext:'승인 작업 계획을 만든 뒤 승인 조건과 증거 연결 방식을 확인하고 진행하세요.',
      safeMode:'권장 시작: 계획만 만들기 또는 이미 있는 결과 파일 분석.',
      evidence:'정규화된 결과만 증거 카드와 주장-증거 연결표에 연결합니다.',
    };
    const selectedToolGuideRows = [
      ['도구 설명', koToolDisplayName(selectedTool.display_name, draft.analysisToolId), selectedToolGuide.summary],
      ...
      ['증거 연결', koCountSummary(selectedTool.evidence_types || [], '증거 유형 미확인'), selectedToolGuide.evidence],
    ];
    ...
    const fileUploadRows = [
      ...
      ['저장 산출물', uploadedArtifact.storage_path ? '저장됨' : '미저장', koHiddenLocation(uploadedArtifact.storage_path)],
    ...
    const executionPlanRows = [
      ...
      ['파일시스템', koValue(executionPlan.environment_constraints?.filesystem_policy?.mode), (executionPlan.environment_constraints?.filesystem_policy?.write_paths || []).length ? '승인된 작업공간만 사용' : '운영 보관소만 사용'],
      ['래퍼', koValue(executionPlan.wrapper_manifest?.pinning_status || selectedWrapper.pinning_status), koBlockerSummary(executionPlan.wrapper_preflight?.blocking_controls || selectedWrapper.runner_preflight?.blocking_controls, '래퍼 신뢰 조건 충족')],
      ['격리', koValue(executionPlan.environment_constraints?.isolation_readiness?.status || draft.runnerBackend), koBlockerSummary(executionPlan.environment_constraints?.isolation_readiness?.blocking_controls, '격리 차단 조건 없음')],
    ...
    const isolationRows = [
      ...
      ['조회 기능의 명령 실행', koBool(isolation.commands_executed_by_api ?? false), '상태 확인 기능은 컨테이너 명령을 직접 실행하지 않음'],
      ...
      ['마운트', koValue(isolation.container_policy?.workspace_mount || 'read_only'), isolation.container_policy?.case_artifact_mount ? '케이스 보관소만 쓰기 허용' : '읽기 전용'],
      ['권한 상승', koBool(isolation.container_policy?.privileged_container_allowed ?? false), '항상 아니오 유지'],
      ['차단 조건', (isolation.blocking_controls || []).length, koBlockerSummary(isolation.blocking_controls)],
    ];
    const runnerRows = [
      ['실행 상태', koValue(runnerRun.status || runnerState.status || '대기'), runnerState.error || runnerRun.run_id || '실행 계획 준비와 실행 토큰 필요'],
      ['시도 결과', koValue(runnerRun.runner_attempt?.status), runnerRun.runner_attempt?.artifact_path ? '실행 기록 보관됨' : String(runnerRun.runner_attempt?.exit_code ?? '아직 실행 안 됨')],
    ...
    const toolchainRows = [
      ...
      ['진행률', toolchainRun.progress_percent != null ? `${toolchainRun.progress_percent}%` : '대기', koEvidenceTerm(toolchainRun.operator_summary_ko || '결과가 준비되면 회수·증거 후보 단계와 검토 우선순위가 표시됩니다')],
      ['다음 행동', koEvidenceTerm(toolchainAnalystFindingReview.next_button_ko || toolchainRun.current_stage_ko || '대기'), koEvidenceTerm(toolchainAnalystFindingReview.next_action_ko || toolchainRun.next_action_ko || '결과 첨부, 결과 회수, 증거 후보 승인 순서로 진행하세요')],
      ...
      ['주장-증거표 초안', koValue(toolchainMatrix.status || toolchainMatrixState.status || '대기'), toolchainMatrixState.error || `${toolchainMatrix.ready_claim_count ?? 0}개 준비 · ${toolchainMatrix.held_claim_count ?? 0}개 보류`],
      ['보고서 v2 초안', koValue(toolchainReportDraft.status || toolchainReportDraftState.status || '대기'), toolchainReportDraftState.error || (toolchainReportDraft.report_generated ? '초안 생성됨 · 최종 내보내기 승인 필요' : '주장-증거표 준비 이후 생성')],
      ['운영 증거 제출 묶음 초안', koValue(operatorEvidenceSubmissionManifest.status || operatorEvidenceSubmissionManifestState.status || '대기'), operatorEvidenceSubmissionManifestState.error || koHiddenLocation(operatorEvidenceSubmissionManifest.submission_manifest_artifact_path)],
      ...
      ['운영 closure 제출 패키지', koValue(operatingClosurePackage.status || operatingClosurePackageState.status || '대기'), operatingClosurePackageState.error || operatingClosurePackage.package_id || '제출 패키지 연결 대기'],
      ['Collection 전체 닫기', koValue(toolchainClosure.status || toolchainClosureState.status || '대기'), toolchainClosureState.error || toolchainClosure.closure_id || '전체 닫기 연결 대기'],
      ...
      ['저장 산출물', toolchainRun.artifact_path ? '저장됨' : '미저장', toolchainRun.artifact_path ? '복합 실행 기록 보관됨' : '복합 실행 후 생성'],
    ].filter(row => showAdminDetails || ![
      '운영 증거 제출 묶음 초안',
    ...
    const installEvidenceCandidateReadyCount = (toolchainRun.install_version_evidence_candidates || []).filter(item => item.status === 'candidate_ready').length;
    ...
    const toolchainAnalystProgressRows = [
      ...
      ['회수 가능', `${toolchainAnalystProgress.collectable_count ?? toolchainRunStatus.collectable_step_count ?? 0}개`, '저장된 실행/첨부 결과 중 증거 후보로 회수 가능한 단계'],
      ['증거 후보', `${toolchainAnalystProgress.evidence_candidate_count ?? toolchainCollection.evidence_candidate_count ?? 0}개`, '승인 전에는 발견사항이나 보고서 주장으로 확정하지 않음'],
    ];
    ...
    const toolchainCollectionRows = (toolchainCollection.steps || []).map(step => [
      ...
      step.evidence_candidate?.evidence_id ? '증거 후보 생성됨' : '증거 후보 대기',
    ]);
    const toolchainAgentSummaryRows = (toolchainCollection.analysis_agent_summaries || []).map(item => [
      ...
      koEvidenceTerm(item.evidence_use_limit_ko || '승인된 Evidence Card와 심각도 승인 전에는 보고서 주장에 사용할 수 없습니다.'),
    ]);
    ...
    const analystFindingMissingRows = (toolchainAnalystFindingReview.missing_tool_rows || []).map(item => [
      item.tool_label_ko || koToolName(item.tool_id) || '필수 도구',
      '결과 누락',
    ...
    const toolchainReportRows = [
      ['초안 상태', koValue(toolchainReportDraft.status || toolchainReportDraftState.status || '대기'), toolchainReportDraftState.error || (toolchainReportDraft.report_request_id ? '보고서 초안 요청됨' : '-')],
      ['보고서 검증', koValue(toolchainReportDraft.report?.gate_status || toolchainReportDraft.validation_preview?.gate_status || '대기'), toolchainReportDraft.report_generated ? '통과' : '연결표 준비 필요'],
      ['최종 내보내기 승인', toolchainReportDraft.requires_final_export_approval ? '별도 필요' : '미확인', '초안 생성은 내보내기 승인이 아님'],
    ];
    const toolchainCompletionRows = [
      ...
      ['운영 closure 제출 패키지', operatingClosurePackage.ready_for_operating_close ? '준비 완료' : koValue(operatingClosurePackage.status || operatingClosurePackageState.status || '대기'), operatingClosurePackage.ready_for_operating_close ? '운영 닫기 요청 검토 가능' : (operatingClosurePackageState.error || '운영 산출물 폴더와 승인자 4명 확인')],
      ['개발 부산물 제외', operatingClosurePackage.completion_evidence_allowed ? '통과' : koValue(operatingClosurePackage.source_completion_review?.allowed_use || '대기'), operatingClosurePackage.completion_evidence_allowed ? '실제 운영 완료 증거 후보' : '샘플/테스트/통제 산출물 경로는 완료/보고서 Claim 증거로 사용하지 않습니다'],
      ...
      ['검토 완료 운영 closure 실행', reviewedOperatingClose.complete ? '완료' : koValue(reviewedOperatingClose.status || reviewedOperatingCloseState.status || '대기'), reviewedOperatingClose.complete ? '사람 검토 요청으로 close 완료' : (reviewedOperatingCloseState.error || '사람 검토 완료 뒤 실행')],
      ['운영 closure 증거 인증', reviewedCloseCertification.ready_for_completion_audit_review ? '완료 감사 후보' : koValue(reviewedCloseCertification.status || reviewedCloseCertificationState.status || '대기'), reviewedCloseCertification.ready_for_completion_audit_review ? '실측 증명 포함' : (reviewedCloseCertificationState.error || '검토된 close 뒤 증거 인증')],
      ...
      ['전체 목표 완료 검토', goalCompletionReview.goal_completion_ready ? '완료 가능' : koValue(goalCompletionReview.status || goalCompletionReviewState.status || '대기'), goalCompletionReview.goal_completion_ready ? '승인 게이트와 종료 조건 통과' : (goalCompletionReviewState.error || `${goalCompletionReview.blocker_count ?? 0}개 차단 조건`)],
      ...
      ['내보내기', toolchainCompletionGate.export_id || '-', `${toolchainCompletionGate.blocker_count ?? 0}개 차단 조건`],
    ];
    const operatingClosureProgressRows = [
      ...
      ['단계 준비', `${operatingClosureProgress.ready_stage_count ?? 0}개 준비`, `차단 ${operatingClosureProgress.blocked_stage_count ?? 0}개 / 누락 도구 ${(operatingClosureProgress.missing_required_tool_ids || []).join(', ') || '없음'}`],
      ['명령 실행', koBool(operatingClosureProgress.commands_executed_by_api ?? false), '요약/검토/감사 기능은 분석도구, 컨테이너, 보조 실행 환경, 네트워크 명령을 실행하지 않음'],
    ];
    const operatingClosureProgressStageRows = (operatingClosureProgress.stage_rows || []).map(item => [
      item.title_ko || item.stage_id || '-',
      koValue(item.status || '대기'),
      item.button_ko || '-',
      item.summary_ko || (item.primary_api ? '연결 준비됨' : '-'),
    ]);
    ...
    const operatingClosureReadinessSummaryRows = [
      ...
      ['다음 연결', koReadyLink(operatingClosureReadinessSummary.next_api), operatingClosureReadinessSummary.does_not_mark_goal_complete ? '전체 목표 완료 처리 없음' : '대기'],
      ['명령 실행', koBool(operatingClosureReadinessSummary.commands_executed_by_api ?? false), '분석도구, 컨테이너, 보조 실행 환경, 네트워크 실행 없음'],
    ];
    const operatingClosureReadinessStepRows = (operatingClosureReadinessSummary.workflow_steps || []).map(item => [
      item.title_ko || item.step_id || '-',
      koValue(item.status || '대기'),
      item.operator_action_ko || '-',
      item.primary_api ? '연결 준비됨' : '-',
    ]);
    const operatingClosureReadinessBlockerRows = (operatingClosureReadinessSummary.blockers || []).map(item => [
      item,
      item === 'real_operating_evidence_readiness_required' ? '실제 운영 증거 사전 점검의 차단 조건을 해소하세요' : (item === 'operating_closure_submission_package_required' ? '운영 closure 제출 패키지를 준비 상태로 만드세요' : '입력값 또는 운영 산출물 폴더를 보완하세요'),
      koReadyLink(operatingClosureReadinessSummary.next_api),
    ]);
    ...
    const realOperatingToolCoverageRows = (realOperatingEvidenceReadiness.tool_coverage || []).map(item => [
      koToolDisplayName(item.display_name, item.tool_id),
      item.status === 'present' ? '산출물 있음' : koValue(item.status || '누락'),
      item.selected_path ? '파일 확인됨' : 'Nuclei/OpenVAS/Trivy/SCA/npm audit/ZAP 6개 결과가 모두 필요',
      item.sha256 || `${item.candidate_count || 0}개 후보`,
    ]);
    ...
    const realOperatingMissingToolRows = (realOperatingEvidenceReadiness.missing_tool_remediation || []).map(item => [
      ...
      item.operator_action_ko || '누락 도구 결과 파일을 운영 산출물 폴더에 추가한 뒤 다시 점검하세요.',
      item.does_not_execute_tool ? '명령 실행 없음' : '확인 필요',
    ]);
    ...
    const operatingClosureReviewRows = (operatingClosureReview.checklist || []).map(item => [
      item.title_ko || item.field || '-',
      koValue(item.status || '대기'),
      item.required ? '필수 확인' : '선택 확인',
    ]);
    const operatingClosureReviewApproverRows = (operatingClosureReview.approver_review || []).map(item => [
      item.role_ko || item.field || '-',
      koValue(item.status || '대기'),
      item.signed_by || item.expected || `${item.field || '승인자'} 서명 필요`,
    ]);
    const reviewedCloseCertificationRows = (reviewedCloseCertification.evidence_checks || []).map(item => [
      item.title_ko || item.field || '-',
      koValue(item.status || '대기'),
      item.evidence || '-',
    ]);
    const reviewedCloseAttestationRows = (reviewedCloseCertification.operator_attestation || []).map(item => [
      item.title_ko || item.field || '-',
      koValue(item.status || '대기'),
      item.required ? '필수 attestation' : '선택',
    ]);
    const operatingCompletionAuditChecklistRows = (operatingCompletionAudit.checklist || []).map(item => [
      item.title_ko || item.field || '-',
      koValue(item.status || '대기'),
      item.evidence || '-',
    ]);
    const operatingCompletionAuditBlockerRows = (operatingCompletionAudit.blockers || []).map(item => [
      item,
    ...
    const visualRows = [
      ...
      ['원본 산출물', visualDescriptor.original_artifact_path ? '저장됨' : '미저장', visualDescriptor.original_artifact_path ? '원본 보관됨' : '이미지 자료 필요'],
      ['마스킹본 산출물', visualDescriptor.redacted_artifact_path ? '저장됨' : '미저장', visualDescriptor.redacted_artifact_path ? '마스킹본 보관됨' : '마스킹 PNG 대기'],
      ['마스킹본 SHA-256', visualDescriptor.redacted_sha256 || '-', visualPreview.visual_bundle?.manifest_path ? '시각 증거 묶음 보관됨' : '시각 증거 묶음 대기'],
    ...
    const agenticRagRows = [
      ...
      ['인용', (agenticRagResult.citations || []).length, (agenticRagResult.citations || []).length ? `${agenticRagResult.citations.length}개 인용 후보` : '승인된 증거 카드 필요'],
      ['근거 부족 주장', agenticVerifier.unsupported_claim_count ?? '-', '보고서 사용 전 0건이어야 함'],
      ['주장-증거 연결표 후보', koValue(agenticMatrixCandidate.status || 'not_prepared'), agenticMatrixCandidate.claim_id ? '후보 준비됨' : 'SCA를 실행해 후보를 준비하세요'],
      ...
      ['선택 말뭉치', (agenticRagResult.selected_corpora || []).length, (agenticRagResult.selected_corpora || []).length ? `${agenticRagResult.selected_corpora.length}개 말뭉치 선택` : '증거 저장소 대기'],
      ['조회 기능의 명령 실행', koBool(agenticRagResult.commands_executed_by_api ?? false), '항상 아니오 유지'],
    ...
    return h('div', { style:{ display:'grid', gap:'14px' } },
      h('div', { style:{ background:C.s1, border:`1px solid ${C.border}`, borderRadius:'12px', padding:'14px' } },
        h('div', { style:{ display:'flex', justifyContent:'space-between', gap:'12px', flexWrap:'wrap', marginBottom:'10px' } },
          h('div', {},
            h('div', { style:{ fontSize:'16px', fontWeight:950 } }, '레드팀 분석2 · RedTeam AX'),
            h('div', { style:{ fontSize:'10.5px', color:C.sec, marginTop:'4px' } }, '승인 작업, 사람 검토, 증거 카드, 주장-증거 연결표를 분리한 분석 워크벤치')),
          h('div', { style:{ display:'flex', gap:'7px', flexWrap:'wrap' } },
            ...
            h('button', { onClick:()=>this.submitRedTeam2ToolActionPlan(), style:{ padding:'8px 11px', borderRadius:'8px', border:`1px solid ${C.blue}`, background:C.blue, color:'#fff', cursor:'pointer', fontWeight:900 } }, '승인 작업 계획'))),
        h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'8px' } }, [
          ['v2 연결', koValue(st.v2Health?.status || st.status || 'idle'), st.v2Health?.status === 'ready' ? C.green : C.sec, st.v2Health?.execution_policy ? koEvidenceTerm(st.v2Health.execution_policy) : '승인 작업 카드 필수'],
          ['v1 백엔드', koValue(st.v1Health?.status || st.v1Health?.service || 'unknown'), C.sec, '기존 레드팀 분석 회귀 기준'],
          ['근거 검색', rag.exists === false ? '대기' : '준비 완료', rag.exists === false ? C.sec : C.green, rag.db_path ? '지식 저장소 연결됨' : (rag.error || '근거 검색 상태')],
          ['준비도', readiness.summary?.ready_count ?? '-', C.blue, readiness.error || '파이프라인 커버리지'],
          ['도구 허브', toolRegistry.tool_count ?? analysisTools.length ?? '-', C.blue, toolRegistry.execution_policy ? koEvidenceTerm(toolRegistry.execution_policy) : '도구 목록을 불러오지 않음'],
        ].map(card))),
        ...
        h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(190px, 1fr))', gap:'10px' } },
          ...
          h('label', { style:{ fontSize:'10.5px', color:C.muted } }, '분석도구',
            h('select', { value:draft.analysisToolId, onChange:e=>this.updateRedTeam2AnalysisDraft({ analysisToolId:e.target.value }), style:{ ...inputStyle, marginTop:'5px' } },
              ...
              ]).map(tool => h('option', { key:tool.tool_id, value:tool.tool_id }, koToolDisplayName(tool.display_name || tool.name, tool.tool_id))))),
          ...
          showAdminDetails ? h('label', { style:{ fontSize:'10.5px', color:C.muted } }, '실행 환경',
            h('select', { value:draft.runnerBackend, onChange:e=>this.updateRedTeam2AnalysisDraft({ runnerBackend:e.target.value }), style:{ ...inputStyle, marginTop:'5px' } },
              [
                ['local_subprocess_shim','로컬 dry-run shim'],
                ['ephemeral_container','임시 컨테이너'],
              ].map(([id,label]) => h('option', { key:id, value:id }, label)))) : null,
      ...
      smallPanel('분석 도구 허브 / LLM 분석 에이전트',
        h('div', { style:{ display:'grid', gap:'10px' } },
          h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'8px' } }, [
            ['선택 도구', koToolDisplayName(selectedTool.display_name, draft.analysisToolId), selectedTool.requires_human_approval ? C.amber : C.green, selectedTool.requires_human_approval ? '승인 범위와 템플릿 확인 필요' : '결과 가져오기 가능'],
            ['LLM 분석 에이전트', selectedTool.llm_agent?.name || (selectedTool.agent_id ? '도구별 분석 에이전트' : '-'), C.blue, '결과 정리와 증거 후보 생성'],
            ['실행 준비', koValue(selectedTool.runtime_status || '미확인'), selectedTool.runtime_status === 'registered_install_required' ? C.amber : C.sec, selectedTool.availability?.command ? '설치 확인 필요' : '결과 가져오기 중심'],
      ...
      smallPanel('분석도구 실행 안내',
        h('div', { style:{ display:'grid', gap:'10px' } },
          h('div', { style:{ fontSize:'11px', color:C.sec, lineHeight:1.55 } }, '이 영역은 버튼을 누르기 전에 확인해야 할 쉬운 설명입니다. 고위험 도구는 웹앱이 바로 실행하지 않고, 승인 작업 계획, 범위 확인, 사람 승인, 결과 업로드, 증거 연결 순서로 진행합니다.'),
          h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'8px' } }, [
            ['선택한 분석도구', koToolDisplayName(selectedTool.display_name, draft.analysisToolId), selectedTool.requires_human_approval ? C.amber : C.green, selectedTool.requires_human_approval ? '승인 후 실행' : '오프라인 분석부터 가능'],
            ...
            ['결과 처리', selectedTool.normalizer_id ? '도구 결과 정규화' : '-', C.teal, selectedTool.agent_id ? 'LLM 분석 에이전트 확인' : 'LLM 분석 에이전트 확인'],
            ['보고서 연결', (selectedTool.evidence_types || []).length || '-', C.green, '증거 카드 생성 후 주장-증거 연결표 연결'],
          ].map(card)),
          this.renderTable(['확인 항목','상태','사용자 안내'], selectedToolGuideRows))),
      smallPanel('근거 검색 충분성 검증 / 인용 검증기',
        h('div', { style:{ display:'grid', gap:'10px' } },
          ...
          h('div', { style:{ display:'flex', gap:'8px', flexWrap:'wrap', alignItems:'center' } },
            ...
            }, agenticRagState.status === 'running' ? '검증 중' : '근거 검색 검증 실행'),
          ...
          h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'8px' } }, [
            ...
            ['증거', agenticRagState.evidence?.evidence_id ? '증거 후보 준비됨' : '-', agenticRagState.evidence?.approval_status === 'approved' ? C.green : C.sec, agenticRagState.evidence?.source_path_or_url ? '증거 카드 후보 보관됨' : '승인된 증거 카드 자동 준비'],
            ['연결표 후보', koValue(agenticMatrixCandidate.status), agenticMatrixCandidate.status === 'ready_for_report_claim' ? C.green : agenticMatrixCandidate.status === 'hold_unsupported_claim' ? C.coral : C.sec, agenticMatrixCandidate.claim_id ? '후보 준비됨' : '아직 없음'],
            ['검색 말뭉치', (agenticRagResult.selected_corpora || []).length || '-', C.blue, (agenticRagResult.selected_corpora || []).slice(0, 3).join(', ') || '검증을 실행하세요'],
          ].map(card)),
          this.renderTable(['근거 검색 검증','상태','근거'], agenticRagRows),
      ...
      smallPanel('분석가용 다음 실행 안내',
        h('div', { style:{ display:'grid', gap:'10px' } },
          h('div', { style:{ fontSize:'11px', color:C.sec, lineHeight:1.55 } },
            '분석가는 결과 첨부, 증거 후보 확인, 보고서 주장 검토 순서로 진행합니다. 준비 세부사항은 관리자 설정에 접어 두었고, 고위험 실행은 사람이 승인한 뒤 진행합니다.'),
          this.renderTable(['분석가 안내','상태','설명'], analystReadinessRows),
          this.renderTable(['다음 버튼','상태','사용자 안내','연결'], analystRoleNextStepRows.length ? analystRoleNextStepRows : [['여러 도구 결과 첨부','대기','운영자가 실행한 결과 파일을 첨부하세요','연결 대기']]),
          ...
          h('div', { style:{ display:'flex', gap:'8px', flexWrap:'wrap', alignItems:'center' } },
            ...
            h('button', { onClick:()=>this.collectRedTeam2ToolchainResults(), style:{ padding:'8px 10px', borderRadius:'8px', border:`1px solid ${C.amber}`, background:C.bg, color:C.amber, cursor:'pointer', fontWeight:900 } }, '결과 회수·증거 후보')),
          this.renderTable(['순서','누를 버튼','무엇을 하는가','연결'], analystExecutionGuideRows),
          this.renderTable(['작업 순서','다음 버튼','상태','필요 입력','차단 사유','연결'], sixToolWorkOrderRows.length ? sixToolWorkOrderRows : [['대기','6개 도구 작업 순서 만들기','대기','케이스/보고서/도구 묶음 ID','연결 호출 전','연결 대기']]),
          this.renderTable(['제출 항목','다음 버튼','예상 파일명','연결','안전'], sixToolSubmissionTemplateRows.length ? sixToolSubmissionTemplateRows : [['대기','6개 도구 제출 양식 만들기','Nuclei/OpenVAS/Trivy/SCA/npm audit/ZAP 결과 파일','연결 대기','명령 실행 없음']]))),
      smallPanel('관리자 설정',
        h('div', { style:{ display:'grid', gap:'10px' } },
          h('div', { style:{ fontSize:'11px', color:C.sec, lineHeight:1.55 } },
            '분석가 기본 화면에서는 준비 세부사항과 운영 절차를 숨깁니다. 플랫폼 운영자나 레드팀 리드가 필요할 때만 펼쳐 확인합니다.'),
          h('button', {
            onClick:()=>this.setState(s => ({ redteam2ShowAdminDetails:!(s.redteam2ShowAdminDetails === true) })),
            style:{ justifySelf:'start', padding:'8px 10px', borderRadius:'8px', border:`1px solid ${showAdminDetails ? C.amber : C.border}`, background:showAdminDetails ? C.bg : C.s2, color:showAdminDetails ? C.amber : C.text, cursor:'pointer', fontWeight:900 },
          }, showAdminDetails ? '관리자 설정 숨기기' : '관리자 설정 보기'),
          showAdminDetails ? null : h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } },
            '숨긴 준비 항목은 분석 결과 판단에 필요한 때 증거 카드와 감사 기록에서 확인합니다.'))),
      showAdminDetails ? smallPanel('도구 래퍼 신뢰 고정 / 버전 확인',
        h('div', { style:{ display:'grid', gap:'10px' } },
          h('div', { style:{ fontSize:'11px', color:C.sec, lineHeight:1.55 } }, '래퍼는 웹앱이 도구를 부를 때 사용하는 중간 실행 파일입니다. 실행 전에 위치, 버전, SHA-256 해시를 확인하고 레드팀 리드가 승인해야 합니다. 승인되지 않은 래퍼는 실행 버튼에서 차단됩니다.'),
          h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'8px' } }, [
            ['래퍼 목록', wrapperRegistry.manifest_count ?? wrapperRows.length ?? '-', C.blue, wrapperRegistry.verification_policy || '래퍼 목록을 불러오세요'],
            ['선택 래퍼', selectedWrapper.resolved_path ? '확인됨' : selectedWrapper.command_name ? '없음' : '가져오기 전용', selectedWrapper.trusted_for_runner ? C.green : C.amber, selectedWrapper.resolved_path ? '경로는 관리자 감사 기록에서 확인' : (selectedWrapper.command_name || '-')],
          ...
          this.renderTable(['도구','고정 상태','사용 가능 여부','해시/위치'], wrapperRows.length ? wrapperRows : [['미로드','-','-','상태 새로고침 필요']]))) : null,
      showAdminDetails ? smallPanel('OpenVAS/ZAP 읽기 전용 접속권한',
        h('div', { style:{ display:'grid', gap:'10px' } },
          h('div', { style:{ fontSize:'11px', color:C.sec, lineHeight:1.55 } }, 'OpenVAS와 ZAP 접속 비밀값은 이 화면에 넣지 않습니다. 외부 비밀 보관 참조만 승인 기록으로 남기고, 범위는 읽기 전용 허용목록과 비교합니다. 능동 스캔이나 관리자/쓰기 권한은 차단됩니다.'),
          h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'8px' } }, [
            ...
            ['조회 기능의 명령 실행', credentialPolicies.commands_executed_by_api ? '예' : '아니오', credentialPolicies.commands_executed_by_api ? C.coral : C.green, '승인 기록만 수행'],
          ].map(card)),
          h('div', { style:{ display:'grid', gridTemplateColumns:'minmax(150px, .7fr) minmax(220px, 1fr) minmax(220px, 1fr)', gap:'8px' } },
            ...
            h('label', { style:{ fontSize:'10.5px', color:C.muted, minWidth:0 } }, '외부 비밀 보관 참조',
              h('input', { value:draft.credentialRef || '', onChange:e=>this.updateRedTeam2AnalysisDraft({ credentialRef:e.target.value }), style:{ ...inputStyle, marginTop:'5px' }, placeholder:'관리자가 등록한 읽기 전용 보관 참조' })),
            h('label', { style:{ fontSize:'10.5px', color:C.muted, minWidth:0 } }, '읽기 전용 접속 참조',
              h('input', { value:draft.credentialEndpointRef || '', onChange:e=>this.updateRedTeam2AnalysisDraft({ credentialEndpointRef:e.target.value }), style:{ ...inputStyle, marginTop:'5px' }, placeholder:'관리자가 등록한 읽기 전용 접속 참조' }))),
          h('div', { style:{ display:'grid', gridTemplateColumns:'minmax(220px, 1fr) minmax(240px, 1.1fr)', gap:'8px' } },
            h('label', { style:{ fontSize:'10.5px', color:C.muted, minWidth:0 } }, '읽기 전용 범위',
              h('input', { value:draft.credentialScopes || '', onChange:e=>this.updateRedTeam2AnalysisDraft({ credentialScopes:e.target.value }), style:{ ...inputStyle, marginTop:'5px' }, placeholder:'보고서 읽기, 스캔 상태 읽기' })),
          ...
          this.renderTable(['승인 기록','상태','근거'], credentialAuthorizationRows))) : null,
      showAdminDetails ? smallPanel('OpenVAS/ZAP 서비스 결과 가져오기',
        h('div', { style:{ display:'grid', gap:'10px' } },
          h('div', { style:{ fontSize:'11px', color:C.sec, lineHeight:1.55 } },
            '읽기 전용 서비스 결과 가져오기는 이미 승인된 OpenVAS 보고서 또는 ZAP passive alert 결과만 회수합니다. 능동 스캔은 실행하지 않습니다. 비밀값은 입력하지 않고, 승인 기록 ID와 읽기 전용 접속 주소만 사용합니다.'),
          h('div', { style:{ display:'grid', gridTemplateColumns:'minmax(150px, .7fr) minmax(220px, 1fr) minmax(240px, 1.1fr) minmax(120px, .5fr)', gap:'8px' } },
            ...
            h('label', { style:{ fontSize:'10.5px', color:C.muted, minWidth:0 } }, '읽기 전용 접속 주소',
              h('input', {
                ...
                style:{ ...inputStyle, marginTop:'5px' },
                placeholder:'관리자가 승인한 읽기 전용 접속 주소',
              })),
          ...
          h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'8px' } }, [
            ...
            ['Evidence', serviceImportEvidence.evidence_id || '-', serviceImportEvidence.evidence_id ? C.green : C.sec, serviceImportEvidence.source_path_or_url ? 'Evidence 후보 보관됨' : 'Evidence 후보'],
            ['저장 산출물', serviceImportArtifact.storage_path ? '저장됨' : '미저장', serviceImportArtifact.storage_path ? C.green : C.sec, serviceImportArtifact.storage_path ? '서비스 결과 원본 보관됨' : (serviceImportArtifact.content_type || '-')],
          ].map(card)),
          ...
          serviceImportArtifact.storage_path ? h('div', { style:{ fontSize:'9.5px', color:C.sec } }, '서비스 결과 원본은 분석 저장소에 보관됨') : null)) : null,
      showAdminDetails ? smallPanel('분석 환경 설정(관리자용)',
        h('div', { style:{ display:'grid', gap:'10px' } },
          h('div', { style:{ fontSize:'11px', color:C.sec, lineHeight:1.55 } },
            '이 영역은 분석 환경 담당자가 보는 세부 설정입니다. 분석가가 바로 이해할 필요가 없는 컨테이너, 보조 실행 환경, 조직 OpenVAS/ZAP 읽기 전용 접속, 외부 비밀 보관 참조, 승격 게이트 상태를 분리해 보여줍니다. 분석가는 위의 분석가용 다음 실행 안내를 먼저 사용하세요.'),
          h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'8px' } }, [
            ...
            ['컨테이너 실행 환경', containerRuntime.runtime_preflight?.ready ? '준비됨' : '차단됨', containerRuntime.runtime_preflight?.ready ? C.green : C.amber, containerRuntime.runtime_preflight?.blocker || koValue(containerRuntime.status) || '상태 미확인'],
            ['보조 실행 환경', koValue(wslRuntime.status || wslRuntimeArtifact.status || '미확인'), ['ready','wsl_runtime_ready'].includes(wslRuntime.status) ? C.green : C.amber, wslRuntime.selected_distro ? '배포판 확인됨' : '보조 실행 환경 준비 확인'],
            ...
            ['복합 도구 결과 회수', '연결 준비됨', C.blue, '저장된 결과만 읽어 정규화와 Evidence Card 후보 생성을 순서대로 수행합니다. 승인 전에는 Finding이나 보고서 Claim으로 확정하지 않습니다'],
            ['6개 도구 작업 순서', '연결 준비됨', C.blue, 'Nuclei/OpenVAS/Trivy/SCA/npm audit/ZAP별 다음 버튼, 필요 입력, 읽기 전용 가져오기 경로를 계산하며 명령은 실행하지 않습니다'],
            ['6개 도구 제출 양식', '연결 준비됨', C.blue, '작업 순서를 운영 증거 제출 묶음용 항목과 첨부 양식으로 변환하며 명령은 실행하지 않습니다'],
            ['운영 산출물 묶음 만들기', '연결 준비됨', C.blue, '운영 산출물 폴더에서 분석도구 결과 파일을 찾아 SHA-256 묶음을 만들고 명령은 실행하지 않습니다'],
            ['운영 산출물 가져오기', '연결 준비됨', C.blue, '파일 위치와 해시를 검증해 Nuclei/OpenVAS/Trivy/SCA/npm audit/ZAP 결과 파일을 한 collection으로 가져옵니다'],
            ['실제 운영 증거 사전 점검', '연결 준비됨', C.amber, '샘플/테스트/통제 산출물을 운영 closure 전에 차단합니다'],
            ['운영 증거 제출 묶음 초안', '연결 준비됨', C.blue, '운영자가 첨부한 파일의 해시와 상태를 확인해 사람 승인 전까지 완료로 보지 않습니다'],
            ['운영 Evidence Card 후보 가져오기', '연결 준비됨', C.blue, '승인된 운영자 제출 증거 후보를 Evidence Card로 등록하고 명시적 사람 검토가 있을 때만 승인 기록을 남깁니다'],
            ['운영 closure 제출 패키지', '연결 준비됨', C.blue, '운영 산출물 폴더, 승인자 4명, 실행 차단 조건, 닫기 요청을 실행 전 검증합니다'],
            ['운영 closure 사람 검토', '연결 준비됨', C.violet, '제출 패키지 체크리스트, 승인자 서명, 차단 조건 처리 방침을 실행 전 기록합니다'],
            ['검토 완료 운영 closure 실행', '연결 준비됨', C.green, '사람 검토 기록의 승인된 닫기 요청만 사용해 별도 HITL close를 실행합니다'],
            ['운영 closure 증거 인증', '연결 준비됨', C.amber, '실제 운영 산출물과 실제 승인자 증명을 completion audit 후보로 검증합니다'],
            ['운영 completion audit 검토', '연결 준비됨', C.amber, '인증 후보를 독립 감사 checklist로 다시 검토하고 테스트 산출물은 완료 후보에서 차단합니다'],
            ['전체 목표 완료 검토', '연결 준비됨', C.coral, 'completion audit matrix, accepted gate, 종료 조건 0건, 개발 부산물 제외를 모두 확인하고 남은 gap이 있으면 완료를 차단합니다'],
            ['복합 Evidence 후보 승인', '연결 준비됨', C.blue, '레드팀 리드 또는 통제팀이 후보 Evidence를 승인해야 Finding 승격과 Matrix 준비로 이동합니다'],
            ['Claim-Evidence Matrix 초안', '연결 준비됨', C.blue, '승인된 Evidence와 2인 severity 승인된 Finding만 보고서 검증 payload에 포함합니다'],
            ['Matrix 기반 Report v2 draft', '연결 준비됨', C.blue, '보류 row 0건과 report gate pass일 때만 한국어 Report v2 draft를 생성합니다'],
            ['복합 Collection 최종 내보내기 게이트', '연결 준비됨', C.amber, 'collection Report v2 draft의 보고서 ID를 최종 게이트 패널에 연결하고 Executive Sponsor 승인 뒤에만 내보냅니다'],
            ['복합 Collection E2E 완료 게이트', '연결 준비됨', C.amber, 'Evidence 승인, Finding 승격, 2인 severity 승인, Matrix ready, Report gate, export 완료를 기존 산출물로만 점검합니다'],
            ['운영 산출물 전체 닫기', '연결 준비됨', C.green, '운영 분석도구 폴더를 묶음으로 만들고 가져오기, 결과 회수, close-e2e를 이어서 수행하되 분석도구 명령은 실행하지 않습니다'],
            ...
            ['실서비스 가져오기', koValue(externalServiceImport.status || externalServiceImportArtifact.status || '미확인'), externalServiceImport.status === 'passed' ? C.green : C.amber, externalServiceImport.service_endpoint_fetch_executed ? '읽기 전용 보고서 가져오기 수행됨' : '아직 조직 읽기 전용 가져오기 미실행'],
            ['상태 조회 실행', runtimeReadiness.commands_executed_by_api ? '명령 실행됨' : '조회만 수행', runtimeReadiness.commands_executed_by_api ? C.coral : C.green, '컨테이너와 분석도구는 이 기능이 직접 실행하지 않음'],
          ].map(card)),
          this.renderTable(['관리자 환경 단계','상태','실행 영향','사용자 조치','확인 방법'], operatorEnvironmentRows.length ? operatorEnvironmentRows : [['대기','미확인','실제 실행 전 필요','런타임 상태 새로고침을 먼저 누르세요','연결 준비됨']]),
          h('div', { style:{ display:'flex', gap:'8px', flexWrap:'wrap', alignItems:'center' } },
          ...
          h('div', { style:{ display:'grid', gap:'6px' } },
            ...
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.55 } },
              '아래 단계는 사람이 순서대로 수행하고, 각 확인 산출물을 Evidence로 첨부해야 합니다. 상태 조회 기능은 이 작업을 대신 실행하지 않습니다.'),
            h('div', { style:{ fontSize:'10px', color:C.muted, lineHeight:1.5 } },
              '기본 순서: 컨테이너 실행 환경 준비 → 보조 실행 환경 복구 → OpenVAS/ZAP 읽기 전용 연결과 외부 비밀 보관 참조 설정 → 읽기 전용 보고서 가져오기 실측 → 최종 실측 승격'),
          ...
          h('div', { style:{ display:'grid', gap:'6px' } },
            ...
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.55 } },
              '제출 묶음의 보관 위치, 파일 해시, 기대 상태, 사람 승인 상태를 읽기 전용으로 확인합니다. 이 검증은 컨테이너, 보조 실행 환경, 분석도구, 네트워크 명령을 실행하지 않습니다.'),
      ...
      smallPanel('도구 실행 계획 / 샌드박스 정책',
        h('div', { style:{ display:'grid', gap:'10px' } },
          showAdminDetails ? h('div', { style:{ fontSize:'11px', color:C.sec, lineHeight:1.55 } }, '실행 계획은 실제 실행 전에 위험도, 승인, 네트워크, 파일 쓰기 위치, 래퍼 신뢰, 실행 토큰을 한 번에 확인하는 단계입니다. PlanReady와 실행 토큰이 없으면 실행 버튼은 비활성화됩니다.') : null,
          showAdminDetails ? h('div', { style:{ display:'flex', gap:'8px', flexWrap:'wrap', alignItems:'center' } },
            h('button', {
            ...
            h('span', { style:{ fontSize:'10px', color:executionPlan.status === 'PlanReady' ? C.green : executionPlan.status === 'approval_required' ? C.amber : executionPlan.status === 'invalid' ? C.coral : C.sec, fontWeight:900 } }, koValue(executionPlan.status || executionPlanState.status || 'idle'))) : null,
          showAdminDetails ? h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'8px' } }, [
            ['계획', executionPlan.execution_plan_id || '-', C.sec, executionPlan.artifact_path ? '계획 산출물 보관됨' : '산출물 대기'],
          ...
          showAdminDetails ? this.renderTable(['통제 항목','상태','근거'], executionPlanRows) : null,
          showAdminDetails ? this.renderTable(['격리 항목','상태','근거'], isolationRows) : null,
          showAdminDetails ? h('div', { style:{ display:'grid', gridTemplateColumns:'minmax(220px, 1fr) auto', gap:'8px', alignItems:'end' } },
            ...
            }, runnerState.status === 'executing' ? '실행 중' : '승인된 실행 시작')) : null,
          showAdminDetails ? this.renderTable(['실행 항목','상태','근거'], runnerRows) : null,
          showAdminDetails && executionPlan.artifact_path ? h('div', { style:{ fontSize:'9.5px', color:C.sec } }, '실행 계획은 분석 저장소에 보관됨') : null,
          showAdminDetails && runnerRun.artifact_path ? h('div', { style:{ fontSize:'9.5px', color:C.sec } }, '실행 기록은 분석 저장소에 보관됨') : null,
          h('div', { style:{ borderTop:`1px solid ${C.border}`, paddingTop:'10px', display:'grid', gap:'10px' } },
            ...
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '이 영역의 목적은 실행 목록을 보여주는 것이 아니라 Nuclei/OpenVAS/Trivy/SCA/npm audit/ZAP 결과를 모아 확인 후보, 심각도 분포, 증거 후보 상태로 정리하는 것입니다. 고위험 실행은 사람이 승인·수행·검토하고, 화면은 승인된 결과를 회수해 분석 흐름으로 넘깁니다.'),
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '안전 설치 확인 버튼은 Nuclei/OpenVAS/Trivy/npm audit/OWASP ZAP의 버전 확인만 자동 구성해 실행합니다. SCA는 결과 첨부 도구라 SBOM, lockfile, 조직 SCA export를 결과 첨부로 제출합니다. 임의 스캔 명령, 능동 스캔, 관리자 준비 항목 실행은 허용하지 않습니다.'),
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '복합 도구 결과 회수는 저장된 출력 또는 운영자 첨부 결과만 읽고, 안전 정리와 도구별 결과 정리를 거친 뒤 증거 카드 후보를 만듭니다. 승인 전에는 발견사항이나 보고서 주장으로 확정하지 않습니다.'),
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '복합 증거 후보 승인은 후보 증거만 승인하며, 발견사항 생성·심각도 승인·보고서 반영은 별도 단계로 남깁니다.'),
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '복합 발견사항 초안 생성은 승인된 증거만 검토 대기 발견사항 초안으로 만들고, 2인 심각도 승인 전에는 주장-증거표와 보고서 주장에 넣지 않습니다.'),
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '복합 발견사항 심각도 2인 승인은 레드팀 리드와 업무 소유자가 같은 심각도를 승인해야 발견사항이 승인되며, 보고서 생성은 주장-증거표 검증 이후에만 진행합니다.'),
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '복합 주장-증거표 초안은 승인된 증거와 2인 승인 발견사항만 준비 항목으로 구성하며 보류 항목은 보고서 입력에서 제외합니다.'),
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '복합 보고서 v2 초안은 주장-증거표 준비와 보고서 검증 통과 시에만 한국어 초안을 생성하고 최종 내보내기 승인은 별도로 남깁니다.'),
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '복합 최종 내보내기 게이트는 보고서 v2 초안의 보고서 ID를 최종 게이트 패널에 자동 연결한 뒤 최종 후원자 승인을 받은 경우에만 내보내기를 허용합니다.'),
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '복합 완료 게이트는 기존 산출물만 읽어 증거 승인, 발견사항 승격, 2인 심각도 승인, 주장-증거표 준비, 보고서 검증, 내보내기 완료를 한 번에 점검합니다.'),
            showAdminDetails ? h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } }, '복합 Collection 전체 닫기는 명시된 사람 승인자 정보를 받아 Evidence 승인, Finding 승격, 2인 severity 승인, Matrix, Report v2 draft, export 승인, export, completion gate를 순서대로 수행하지만 분석도구 명령과 능동 스캔은 실행하지 않습니다.') : null,
            ...
            h('div', { style:{ display:'grid', gridTemplateColumns:'minmax(180px, .8fr) minmax(240px, 1.2fr)', gap:'8px' } },
              h('label', { style:{ fontSize:'10.5px', color:C.muted, minWidth:0 } }, '복합 처리 방식',
                h('select', {
                  ...
                  h('option', { value:'runner' }, '승인된 로컬 실행'))),
            ...
            draft.compositeInputMode === 'runner'
              ...
              : h('div', { style:{ display:'grid', gap:'8px' } },
                  h('label', { style:{ fontSize:'10.5px', color:C.muted, minWidth:0 } }, '운영자 결과 본문',
                   ...
                   h('div', { style:{ fontSize:'10px', color:C.sec, lineHeight:1.45 } }, '구분선 하나가 다음 도구 결과의 시작입니다. 이 입력은 명령으로 신뢰하지 않고 증거 후보 전 안전 정리와 도구별 결과 정리를 통과합니다.')),
            showAdminDetails ? h('div', { style:{ borderTop:`1px solid ${C.border}`, paddingTop:'8px', display:'grid', gap:'8px' } },
              ...
              h('label', { style:{ fontSize:'10.5px', color:C.muted, minWidth:0 } }, '운영 산출물 폴더 경로',
                h('input', {
                  ...
                  style:{ ...inputStyle, marginTop:'5px', fontFamily:'ui-monospace, SFMono-Regular, Menlo, Consolas, monospace' },
                  placeholder:'관리자가 승인한 운영 산출물 폴더',
                })),
              h('textarea', {
                ...
                placeholder:'운영 산출물 묶음 만들기 버튼으로 자동 생성하거나 관리자가 검증한 JSON을 붙여넣으세요.',
              })) : null,
            showAdminDetails ? h('div', { style:{ borderTop:`1px solid ${C.border}`, paddingTop:'8px', display:'grid', gap:'8px' } },
              h('div', { style:{ fontSize:'11px', color:C.text, fontWeight:900 } }, '운영 산출물 전체 닫기'),
              h('div', { style:{ fontSize:'10px', color:C.sec, lineHeight:1.45 } }, '이미 사람이 승인 범위에서 실행해 저장한 분석도구 산출물 폴더만 사용합니다. 먼저 운영 closure 제출 패키지를 만들고, 사람 검토 기록으로 체크리스트와 승인자 서명을 남긴 뒤 전체 닫기를 실행합니다. 이 경로는 도구 명령을 실행하지 않습니다.'),
              h('label', { style:{ fontSize:'10.5px', color:C.muted, minWidth:0 } }, '전체 닫기용 운영 분석도구 산출물 폴더',
                h('input', {
                  ...
                  style:{ ...inputStyle, marginTop:'5px', fontFamily:'ui-monospace, SFMono-Regular, Menlo, Consolas, monospace' },
                  placeholder:'관리자가 승인한 운영 분석도구 산출물 폴더',
                })),
              h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(190px, 1fr))', gap:'8px' } },
                h('label', { style:{ fontSize:'10.5px', color:C.muted, minWidth:0 } }, '운영 증거 제출자',
                  h('input', {
                    value:draft.operatorEvidenceSubmissionOperator || draft.compositeClosureReviewer || '',
                    onChange:e=>this.updateRedTeam2AnalysisDraft({ operatorEvidenceSubmissionOperator:e.target.value }),
                    style:{ ...inputStyle, marginTop:'5px' },
                    placeholder:'operator@example.com',
                  })),
                h('label', { style:{ fontSize:'10.5px', color:C.muted, minWidth:0 } }, 'ROE 승인 참조',
                  h('input', {
                    value:draft.operatorEvidenceSubmissionRoeReference || '',
                    onChange:e=>this.updateRedTeam2AnalysisDraft({ operatorEvidenceSubmissionRoeReference:e.target.value }),
                    style:{ ...inputStyle, marginTop:'5px' },
                    placeholder:'ROE-APPROVED-...',
                  }))),
              h('label', { style:{ fontSize:'10.5px', color:C.muted, minWidth:0 } }, '운영 증거 제출 첨부 JSON',
                h('textarea', {
                  value:draft.operatorEvidenceSubmissionAttachmentsJson || '',
                  onChange:e=>this.updateRedTeam2AnalysisDraft({ operatorEvidenceSubmissionAttachmentsJson:e.target.value }),
                  rows:6,
                  style:{ ...inputStyle, marginTop:'5px', resize:'vertical', fontFamily:'ui-monospace, SFMono-Regular, Menlo, Consolas, monospace' },
                  placeholder:'운영 증거 제출 양식 만들기 버튼으로 자동 입력하거나, 관리자가 검증한 첨부 JSON을 붙여넣으세요.',
                })),
              h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(190px, 1fr))', gap:'8px' } },
              ...
              h('label', { style:{ display:'flex', gap:'8px', alignItems:'center', fontSize:'10.5px', color:C.text, fontWeight:800 } },
                ...
                '사람 검토를 완료했으며 생성된 Evidence Card를 바로 승인 기록으로 남김')) : null,
            showAdminDetails ? h('div', { style:{ borderTop:`1px solid ${C.border}`, paddingTop:'8px', display:'grid', gap:'8px' } },
              ...
              h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(190px, 1fr))', gap:'8px' } },
                ...
                h('label', { style:{ fontSize:'10.5px', color:C.muted, minWidth:0 } }, '최종 후원자',
                  h('input', { value:draft.compositeClosureExportApprover || '', onChange:e=>this.updateRedTeam2AnalysisDraft({ compositeClosureExportApprover:e.target.value }), style:{ ...inputStyle, marginTop:'5px' } })))) : null,
            h('div', { style:{ display:'flex', gap:'8px', flexWrap:'wrap', alignItems:'center' } },
              ...
              }, realOperatingEvidenceReadinessState.status === 'checking' ? '실제 증거 점검 중' : '실제 운영 증거 사전 점검') : null,
              showAdminDetails ? h('button', {
              ...
              }, operatingClosureReadinessSummaryState.status === 'summarizing' ? '준비 요약 중' : '운영 closure 준비 요약') : null,
              showAdminDetails ? h('button', {
              ...
              }, operatorEvidenceSubmissionManifestState.status === 'drafting' ? '제출 묶음 초안 중' : '운영 증거 제출 묶음 초안') : null,
              h('button', {
              ...
              }, operatorEvidenceCardImportState.status === 'importing' ? 'Evidence Card 등록 중' : '운영 Evidence Card 후보 import') : null,
              showAdminDetails ? h('button', {
              ...
              showAdminDetails ? h('button', {
                ...
                disabled:installCandidateAttestationState.status === 'recording' || !installEvidenceCandidateReadyCount,
                style:{ padding:'8px 10px', borderRadius:'8px', border:`1px solid ${C.amber}`, background:(installCandidateAttestationState.status === 'recording' || !installEvidenceCandidateReadyCount) ? C.raised : C.bg, color:(installCandidateAttestationState.status === 'recording' || !installEvidenceCandidateReadyCount) ? C.muted : C.amber, cursor:(installCandidateAttestationState.status === 'recording' || !installEvidenceCandidateReadyCount) ? 'not-allowed' : 'pointer', fontWeight:900 },
              }, installCandidateAttestationState.status === 'recording' ? '설치 증거 기록 중' : `후보 ${installEvidenceCandidateReadyCount || 0}개 검토 후 설치 증거 기록`) : null,
              h('button', {
              ...
              }, toolchainState.status === 'executing' ? '결과 처리 중' : (draft.compositeInputMode === 'operator_import' ? '분석 결과 첨부' : '승인된 분석 실행 시작')),
              h('button', {
              ...
              }, toolchainState.status === 'manifest-building' ? '제출 묶음 만드는 중' : '폴더에서 제출 묶음 만들기') : null,
              showAdminDetails ? h('button', {
              ...
              }, toolchainState.status === 'manifest-importing' ? '제출 묶음 가져오는 중' : '운영 산출물 제출 묶음 가져오기') : null,
              h('button', {
              ...
              }, toolchainCollectionState.status === 'collecting' ? '결과 회수 중' : '결과 회수·증거 후보'),
              h('button', {
              ...
              }, toolchainEvidenceApprovalState.status === 'approving' ? '증거 승인 중' : '증거 후보 승인'),
              h('button', {
              ...
              }, toolchainFindingPromotionState.status === 'promoting' ? '발견사항 생성 중' : '발견사항 초안 생성'),
              h('button', {
              ...
              }, toolchainFindingSeverityState.status === 'approving' ? '심각도 승인 중' : '발견사항 심각도 2인 승인'),
              h('button', {
              ...
              }, toolchainMatrixState.status === 'building' ? '주장-증거표 구성 중' : '주장-증거표 초안 생성'),
              h('button', {
              ...
              }, toolchainReportDraftState.status === 'generating' ? '보고서 생성 중' : '보고서 v2 초안 생성'),
              showAdminDetails ? h('button', {
              ...
              }, toolchainClosureState.status === 'closing' ? '전체 닫는 중' : '전체 닫기: 승인·보고서·Export') : null,
              showAdminDetails ? h('button', {
              ...
              }, operatingClosurePackageState.status === 'preparing' ? '제출 패키지 확인 중' : '운영 closure 제출 패키지 확인') : null,
              showAdminDetails ? h('button', {
                onClick:()=>this.recordRedTeam2OperatingClosureHumanReview(),
                disabled:operatingClosureReviewState.status === 'recording' || !operatingClosurePackage.package_id,
                style:{ padding:'8px 10px', borderRadius:'8px', border:`1px solid ${C.violet}`, background:operatingClosureReviewState.status === 'recording' ? C.raised : C.bg, color:(operatingClosureReviewState.status === 'recording' || !operatingClosurePackage.package_id) ? C.muted : C.violet, cursor:(operatingClosureReviewState.status === 'recording' || !operatingClosurePackage.package_id) ? 'not-allowed' : 'pointer', fontWeight:900 },
              }, operatingClosureReviewState.status === 'recording' ? '사람 검토 기록 중' : '운영 closure 사람 검토 기록') : null,
              showAdminDetails ? h('button', {
                onClick:()=>this.executeRedTeam2ReviewedOperatingClose(),
                disabled:reviewedOperatingCloseState.status === 'reviewed-closing' || !operatingClosureReview.review_id,
                style:{ padding:'8px 10px', borderRadius:'8px', border:`1px solid ${C.green}`, background:reviewedOperatingCloseState.status === 'reviewed-closing' ? C.raised : C.bg, color:(reviewedOperatingCloseState.status === 'reviewed-closing' || !operatingClosureReview.review_id) ? C.muted : C.green, cursor:(reviewedOperatingCloseState.status === 'reviewed-closing' || !operatingClosureReview.review_id) ? 'not-allowed' : 'pointer', fontWeight:900 },
              }, reviewedOperatingCloseState.status === 'reviewed-closing' ? '검토 완료 closure 실행 중' : '검토 완료 운영 closure 실행') : null,
              showAdminDetails ? h('button', {
                onClick:()=>this.certifyRedTeam2ReviewedOperatingCloseEvidence(),
                disabled:reviewedCloseCertificationState.status === 'certifying' || !reviewedOperatingClose.execution_id,
                style:{ padding:'8px 10px', borderRadius:'8px', border:`1px solid ${C.amber}`, background:reviewedCloseCertificationState.status === 'certifying' ? C.raised : C.bg, color:(reviewedCloseCertificationState.status === 'certifying' || !reviewedOperatingClose.execution_id) ? C.muted : C.amber, cursor:(reviewedCloseCertificationState.status === 'certifying' || !reviewedOperatingClose.execution_id) ? 'not-allowed' : 'pointer', fontWeight:900 },
              }, reviewedCloseCertificationState.status === 'certifying' ? '증거 인증 중' : '운영 closure 증거 인증') : null,
              showAdminDetails ? h('button', {
                onClick:()=>this.reviewRedTeam2OperatingCompletionAuditCandidate(),
                disabled:operatingCompletionAuditState.status === 'auditing' || !reviewedCloseCertification.certification_id,
              ...
              }, operatingCompletionAuditState.status === 'auditing' ? '완료 감사 검토 중' : '운영 completion audit 검토') : null,
              showAdminDetails ? h('button', {
              ...
              }, goalCompletionReviewState.status === 'reviewing' ? '전체 목표 검토 중' : '전체 목표 완료 검토') : null,
              showAdminDetails ? h('button', {
              ...
              }, toolchainClosureState.status === 'operating-closing' ? '운영 산출물 닫는 중' : '운영 산출물 전체 닫기') : null,
              showAdminDetails ? h('button', {
            ...
            h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5, margin:'4px 0 8px' } },
              ...
              '도구 결과는 승인 전까지 참고 후보입니다. 세부 실행 ID와 저장 위치는 관리자 감사 기록과 증거 카드에서 추적합니다.'),
            showAdminDetails ? this.renderTable(['상세 실행 기록(관리자/감사용)','상태','보관 근거','출력'], toolchainStepRows.length ? toolchainStepRows : [['대기','-','승인된 분석 실행 또는 도구 결과 첨부 뒤 표시됩니다','-']]) : null,
            ...
            this.renderTable(['발견사항 ID','심각도 승인 상태','리드/업무 승인','결과'], toolchainFindingSeverityRows.length ? toolchainFindingSeverityRows : [['대기','-','발견사항 초안 생성 뒤 심각도 2인 승인 버튼을 누르세요','-']]),
            this.renderTable(['발견사항 ID','주장-증거표 상태','주장 ID','차단/결과'], toolchainMatrixRows.length ? toolchainMatrixRows : [['대기','-','심각도 승인 뒤 주장-증거표 초안 생성 버튼을 누르세요','-']]),
            this.renderTable(['보고서 v2','상태','근거'], toolchainReportRows),
            showAdminDetails ? this.renderTable(['운영 closure 진행 요약','상태','설명'], operatingClosureProgressRows) : null,
            showAdminDetails ? this.renderTable(['운영 closure 단계','상태','다음 버튼','설명'], operatingClosureProgressStageRows.length ? operatingClosureProgressStageRows : [['대기','-','운영 closure 준비 요약','실제 운영 증거 사전 점검 또는 운영 closure 준비 요약 버튼을 누르세요']]) : null,
            showAdminDetails ? this.renderTable(['실제 운영 증거 사전 점검','상태','근거'], realOperatingEvidenceReadinessRows.length ? realOperatingEvidenceReadinessRows : [['대기','-','실제 운영 증거 사전 점검 버튼을 누르세요']]) : null,
            showAdminDetails ? this.renderTable(['운영 closure 준비 요약','상태','근거'], operatingClosureReadinessSummaryRows) : null,
            showAdminDetails ? this.renderTable(['운영 closure 다음 단계','상태','사용자 조치','연결'], operatingClosureReadinessStepRows.length ? operatingClosureReadinessStepRows : [['대기','-','운영 closure 준비 요약 버튼을 누르세요','연결 대기']]) : null,
            showAdminDetails ? this.renderTable(['운영 closure 준비 차단 조건','조치','다음 연결'], operatingClosureReadinessBlockerRows.length ? operatingClosureReadinessBlockerRows : [['없음','현재 표시된 준비 차단 조건 없음','-']]) : null,
            showAdminDetails ? this.renderTable(['필수 분석도구 산출물','상태','파일','SHA-256/후보'], realOperatingToolCoverageRows.length ? realOperatingToolCoverageRows : [['대기','-','Nuclei/OpenVAS/Trivy/SCA/npm audit/ZAP 6개 결과 파일을 점검합니다','-']]) : null,
            showAdminDetails ? this.renderTable(['누락 도구','예상 파일명 패턴','다음 행동','안전'], realOperatingMissingToolRows.length ? realOperatingMissingToolRows : [['없음','-','현재 표시된 누락 도구 없음','-']]) : null,
            showAdminDetails ? this.renderTable(['실제 운영 증거 차단 조건','조치','경고'], realOperatingEvidenceBlockerRows.length ? realOperatingEvidenceBlockerRows : [['없음','현재 표시된 차단 조건 없음','-']]) : null,
            showAdminDetails ? this.renderTable(['운영 증거 제출 묶음 항목','상태','파일/해시/검토'], operatorEvidenceSubmissionManifestRows.length ? operatorEvidenceSubmissionManifestRows : [['대기','-','운영 증거 제출 묶음 초안 버튼을 누르세요']]) : null,
            showAdminDetails ? this.renderTable(['운영 증거 제출 묶음 누락','항목','조치'], operatorEvidenceSubmissionManifestBlockerRows.length ? operatorEvidenceSubmissionManifestBlockerRows : [['없음','현재 표시된 누락 없음','-']]) : null,
            showAdminDetails ? this.renderTable(['운영 Evidence Card import','상태','근거'], operatorEvidenceCardImportSummaryRows) : null,
            showAdminDetails ? this.renderTable(['Evidence Card ID','승인 상태','원본/저장 위치'], operatorEvidenceCardImportRows.length ? operatorEvidenceCardImportRows : [['대기','-','운영 Evidence Card 후보 import 버튼을 누르세요']]) : null,
            showAdminDetails ? this.renderTable(['운영 closure 제출 항목','상태','근거'], operatingClosurePackageRows.length ? operatingClosurePackageRows : [['대기','-','운영 closure 제출 패키지 확인 버튼을 누르세요']]) : null,
            showAdminDetails ? this.renderTable(['운영 closure 승인자','상태','입력값'], operatingClosureApproverRows.length ? operatingClosureApproverRows : [['대기','-','승인자 4명 입력 필요']]) : null,
            showAdminDetails ? this.renderTable(['운영 closure 사람 검토','상태','설명'], operatingClosureReviewRows.length ? operatingClosureReviewRows : [['대기','-','제출 패키지 확인 뒤 사람 검토 기록 버튼을 누르세요']]) : null,
            showAdminDetails ? this.renderTable(['운영 closure 서명','상태','서명자'], operatingClosureReviewApproverRows.length ? operatingClosureReviewApproverRows : [['대기','-','승인자 서명 기록 필요']]) : null,
            showAdminDetails ? this.renderTable(['운영 closure 증거 검사','상태','근거'], reviewedCloseCertificationRows.length ? reviewedCloseCertificationRows : [['대기','-','검토 완료 운영 closure 실행 뒤 증거 인증 버튼을 누르세요']]) : null,
            showAdminDetails ? this.renderTable(['운영 closure 실측 attestation','상태','설명'], reviewedCloseAttestationRows.length ? reviewedCloseAttestationRows : [['대기','-','실제 운영 산출물과 승인자 확인 필요']]) : null,
            showAdminDetails ? this.renderTable(['운영 completion audit checklist','상태','근거'], operatingCompletionAuditChecklistRows.length ? operatingCompletionAuditChecklistRows : [['대기','-','운영 closure 증거 인증 뒤 완료 감사 검토 버튼을 누르세요']]) : null,
            showAdminDetails ? this.renderTable(['운영 completion audit 차단 조건','조치','경고'], operatingCompletionAuditBlockerRows.length ? operatingCompletionAuditBlockerRows : [['없음','현재 표시된 차단 조건 없음','-']]) : null,
            showAdminDetails ? this.renderTable(['전체 목표 완료 checklist','상태','근거'], goalCompletionChecklistRows.length ? goalCompletionChecklistRows : [['대기','-','전체 목표 완료 검토 버튼을 누르세요']]) : null,
            showAdminDetails ? this.renderTable(['전체 목표 완료 차단 조건','조치','남은 수'], goalCompletionBlockerRows.length ? goalCompletionBlockerRows : [['없음','현재 표시된 차단 조건 없음','-']]) : null,
            showAdminDetails ? this.renderTable(['E2E 게이트','상태','근거'], toolchainCompletionRows) : null),
      ...
      smallPanel('도구 출력 안전 정리 미리보기',
        h('div', { style:{ display:'grid', gap:'10px' } },
          h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } },
            '도구 결과는 AI에게 내리는 명령이 아니라 분석 자료입니다. 먼저 프롬프트 인젝션, 비밀값, 마스킹 필요 항목을 확인한 뒤 증거 후보로만 연결합니다.'),
          ...
          h('div', { style:{ display:'flex', gap:'8px', flexWrap:'wrap', alignItems:'center' } },
            ...
            }, sanitizerState.status === 'running' ? '미리보기 실행 중' : '안전 정리 미리보기'),
            h('button', {
          ...
          h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'8px' } }, [
            ['안전 정리', koValue(sanitizerPreview.status || sanitizer.decision), sanitizerColor, sanitizerPreview.preview_id ? '미리보기 산출물 생성' : (sanitizerState.error || '미리보기 산출물')],
            ['처리 방식', sanitizerState.run?.run_id ? '저장 기록 있음' : '-', C.sec, koValue(sanitizerState.run?.status) || '첨부 결과만 읽음'],
      ...
      smallPanel('시각 증거 OCR 마스킹 미리보기',
        h('div', { style:{ display:'grid', gap:'10px' } },
          ...
          h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'8px' } }, [
            ...
            ['SHA-256', visualRedactionState.sha256 || visualPreview.source?.sha256 || '-', C.sec, '전송 전에 브라우저에서 계산'],
      ...
      smallPanel('도구 결과 파일 업로드',
        h('div', { style:{ display:'grid', gap:'10px' } },
          h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } },
            'Nuclei, OpenVAS, Trivy, SCA, npm audit, OWASP ZAP 결과 파일을 업로드하면 해시와 스키마를 확인한 뒤 증거 후보로 정리합니다. 업로드 파일도 LLM 명령으로 신뢰하지 않습니다.'),
          h('div', { style:{ display:'flex', gap:'8px', flexWrap:'wrap', alignItems:'center' } },
          ...
          uploadedArtifact.storage_path ? h('div', { style:{ fontSize:'9.5px', color:C.sec } }, '업로드 산출물은 분석 저장소에 보관됨') : null,
          fileUploadState.error ? h('div', { style:{ fontSize:'10.5px', color:C.coral } }, fileUploadState.error) : null)),
      smallPanel('케이스 권한 정책',
        h('div', { style:{ display:'grid', gap:'10px' } },
          h('div', { style:{ fontSize:'10.5px', color:C.sec, lineHeight:1.5 } },
          ...
          h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(180px, 1fr))', gap:'10px' } },
            h('label', { style:{ fontSize:'10.5px', color:C.muted } }, '권한 사용자',
            ...
            h('label', { style:{ fontSize:'10.5px', color:C.muted } }, '권한 역할',
          ...
          h('div', { style:{ display:'flex', gap:'8px', flexWrap:'wrap' } },
            h('button', { onClick:()=>this.loadRedTeam2AnalysisStatus(), style:{ padding:'8px 10px', borderRadius:'8px', border:`1px solid ${C.border}`, background:C.bg, color:C.text, cursor:'pointer', fontWeight:900 } }, '권한 불러오기'),
          ...
          this.renderTable(['사용자','역할','권한'], rbacRows.length ? rbacRows : [['아직 불러오지 않음','-','권한 불러오기 또는 기본 정책 적용']]))),
      smallPanel('평가 맥락',
        h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(190px, 1fr))', gap:'8px' } }, [
      ...
      smallPanel('보고서 v2 최종 게이트 / 내보내기',
        h('div', { style:{ display:'grid', gap:'10px' } },
          reportState.collectionReportDraft?.collection_id ? h('div', { style:{ fontSize:'10.5px', color:C.green, lineHeight:1.5 } },
            `복합 결과 묶음 보고서 v2 초안이 최종 게이트에 연결됨: ${reportState.collectionReportDraft.collection_id} · ${reportState.report?.report_id || '-'}`) : null,
          h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(180px, 1fr))', gap:'10px' } },
            ...
            h('label', { style:{ fontSize:'10.5px', color:C.muted } }, '증거 ID',
            ...
            h('label', { style:{ fontSize:'10.5px', color:C.muted } }, '발견사항 ID',
          ...
          h('div', { style:{ display:'flex', gap:'8px', flexWrap:'wrap' } },
            h('button', { onClick:()=>this.generateRedTeam2ReportDraft(), style:{ padding:'8px 10px', borderRadius:'8px', border:`1px solid ${C.blue}`, background:C.blue, color:'#fff', cursor:'pointer', fontWeight:900 } }, '보고서 v2 초안 생성'),
          ...
          reportResult.report?.artifact_path ? h('div', { style:{ fontSize:'9.5px', color:C.sec } }, '보고서 초안은 분석 저장소에 보관됨') : null,
          reportExported.artifact_path ? h('div', { style:{ fontSize:'9.5px', color:C.green } }, '내보내기 산출물은 분석 저장소에 보관됨') : null)),
      queueCards.length ? smallPanel(`승인 작업 대기열${st.queue?.count ? ` · ${st.queue.count}` : ''}`, h('div', { style:{ display:'grid' } }, queueCards)) : null,
  ...
  reportsList() {
    ...
    return h('div', { style:{ height:'100%', overflow:'auto', padding:'18px' } },
      h('div', { style:{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'8px' } },
        h('div', { style:{ fontSize:'15px', fontWeight:700 } }, '보고서 스튜디오'),
      ...
      h('div', { style:{ fontSize:'11.5px', color:C.muted, marginBottom:'16px', maxWidth:'900px', lineHeight:1.5 } }, '보고서 스튜디오는 백엔드 분석 런과 증거 그래프를 기준으로 초안을 만듭니다. 레드팀 보고서는 공개 보고서 사례의 절차 구조를 참고하되 실제 근거는 레드팀 스튜디오, 안전한 표면 점검, 도구 실행·차단 결정, 증거 연결표, 최종 승인 게이트에서만 가져오며 화면에는 분석가가 검토할 요약, 증거, 제한사항, 대응 권고, 승인 상태를 노출합니다.'),
