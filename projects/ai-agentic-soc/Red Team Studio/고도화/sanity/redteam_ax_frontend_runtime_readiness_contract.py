...
def main() -> int:
    ...
    required_panel_terms = [
        ...
        "실제 운영 증거 사전 점검 API",
        "/api/redteam/v2/toolchains/real-operating-evidence-readiness",
        "운영 증거 제출 manifest 초안 API",
        "/api/redteam/v2/toolchains/operator-evidence-submission-manifest-draft",
        "운영자가 첨부한 artifact_path의 sha256과 status를 확인해 validator용 submission_manifest 초안을 만들고",
        "운영 Evidence Card 후보 import API",
        "/api/redteam/v2/toolchains/operator-evidence-card-import",
        "승인된 operator 제출 증거 후보를 Evidence Card로 등록하고 명시적 사람 검토가 있을 때만 승인 기록을 남깁니다",
        "운영 closure 제출 패키지 API",
        "/api/redteam/v2/toolchains/operating-closure-submission-package",
        "운영 closure 사람 검토 API",
        "/api/redteam/v2/toolchains/operating-closure-human-review",
        "검토 완료 운영 closure 실행 API",
        "/api/redteam/v2/toolchains/execute-reviewed-operating-close",
        "운영 closure 증거 인증 API",
        "/api/redteam/v2/toolchains/certify-reviewed-operating-close-evidence",
        "운영 completion audit 검토 API",
        ...
        "남은 gap이 있으면 완료를 차단",
        "source_dir, 승인자 4명, runtime blocker, close-operating payload를 실행 전 검증합니다",
        "제출 패키지 체크리스트, 승인자 서명, blocker 처리 방침을 실행 전 기록합니다",
        "사람 검토 record의 승인된 close payload만 사용해 별도 HITL close를 실행합니다",
        "실제 운영 산출물과 실제 승인자 attestation을 completion audit 후보로 검증합니다",
        "인증 후보를 독립 감사 checklist로 다시 검토하고 controlled/test 산출물은 완료 후보에서 차단합니다",
        "CASE-V2, fixture, operator-scanner-outputs 같은 테스트 경로를 운영 closure 전에 차단합니다",
        "Finding 초안 생성 API",
    ...
    required_safety_terms = [
        ...
        "도구 진행",
        "사용자 안내",
        "여러 분석도구 실행 버튼을 누르면 도구별 진행 상태가 표시됩니다",
        "LLM 분석 에이전트 요약",
        "증거 사용 제한",
        "결과 회수 뒤 표시됩니다",
        ...
        "운영 closure 다음 단계",
        "운영 closure 준비 blocker",
        "운영 closure 사람 검토 기록 가능",
        "scanner, Docker, WSL, network 실행 없음",
        ...
        "누락 도구 결과 파일을 source_dir에 추가한 뒤 다시 점검하세요",
        "6개 결과가 모두 필요",
        "다음 실행 준비 단계",
        "화면 버튼",
        "도구 실행 가능 여부",
        ...
        "partial_safe_local_smoke",
        "안전 smoke 부분 실행",
        "executeRedTeam2SafeLocalSmokeToolchain",
        "안전 설치 확인 smoke",
        "safe_local_smoke_button",
        "version-only 명령만 자동 구성",
        "임의 스캔 명령, active scan, Docker/WSL/network 실행은 허용하지 않습니다",
        "전체 운영 readiness가 막혀도 version-only 로컬 smoke만 제한적으로 허용하고 임의 스캔 명령은 차단",
        "frontend_action_key",
        "redteam2_button_ko",
        "next_action_plan",
        "운영 증거 제출 manifest 초안 API",
        ...
        "운영 증거 제출 manifest 항목",
        "운영 증거 제출 manifest 누락",
        "실제 운영 증거 blocker",
        "운영 closure 사람 검토 기록",
        "검토 완료 운영 closure 실행",
        "운영 closure 증거 인증",
