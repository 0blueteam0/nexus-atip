...
REQUIRED_KOREAN_ANCHORS = [
    ...
    "Finding 심각도 2인 승인",
    "red_team_lead와 business_owner가 같은 severity를 승인해야",
    ...
    "실행 전 readiness",
    "실행 전 준비 차단",
    "설치 확인, wrapper pin, 격리 준비 상태를 먼저 확인하세요",
    ...
    "남은 gap이 있으면 완료를 차단",
    "source_dir, 승인자 4명, runtime blocker, close-operating payload를 먼저 검증",
    "제출 패키지 체크리스트, 승인자 서명, blocker 처리 방침을 실행 전 기록",
    "사람 검토 record의 승인된 close payload만 사용해 별도 HITL close를 실행",
    "실제 운영 산출물과 실제 승인자 attestation을 completion audit 후보로 검증",
    "인증 후보를 독립 감사 checklist로 다시 검토하고 controlled/test 산출물은 완료 후보에서 차단",
    "CASE-V2, fixture, operator-scanner-outputs 같은 테스트 경로를 운영 closure 전에 차단",
    ...
    "운영 closure 사람 검토",
    "운영 closure 서명",
    "기존 운영 scanner 폴더를 manifest로 만들고 가져오기, 결과 회수, close-e2e까지 이어서 수행하지만 scanner, Docker, WSL, 네트워크 스캔 명령은 실행하지 않습니다",
