...
mklink /D "C:\tmp" "K:\PortableApps\genai\tmp" 2>nul
mklink /D "C:\temp\claude" "K:\PortableApps\genai\tmp" 2>nul

REM 2. 스냅샷 디렉토리 자동 정리 및 재생성
echo [2/5] Fixing snapshot directory...
if exist "K:\PortableApps\genai\shell-snapshots" (
    rmdir /S /Q "K:\PortableApps\genai\shell-snapshots"
)
mkdir "K:\PortableApps\genai\shell-snapshots"

REM 3. 더미 스냅샷 파일 생성 (에러 방지)
echo [3/5] Creating dummy snapshots...
echo #!/bin/bash > "K:\PortableApps\genai\shell-snapshots\snapshot-bash-dummy.sh"
echo # Dummy snapshot file >> "K:\PortableApps\genai\shell-snapshots\snapshot-bash-dummy.sh"

REM 4. 환경 변수 영구 설정
echo [4/5] Setting permanent environment variables...
setx TMPDIR "K:\PortableApps\genai\tmp" >nul 2>&1
setx TEMP "K:\PortableApps\genai\tmp" >nul 2>&1
setx TMP "K:\PortableApps\genai\tmp" >nul 2>&1
setx CLAUDE_HOME "K:\PortableApps\genai" >nul 2>&1
setx CLAUDE_SHELL_SNAPSHOTS "K:\PortableApps\genai\shell-snapshots" >nul 2>&1
setx NODE_OPTIONS "--max-old-space-size=4096" >nul 2>&1
setx MCP_TIMEOUT "60000" >nul 2>&1

REM 5. 권한 문제 해결
echo [5/5] Fixing permissions...
icacls "K:\PortableApps\genai" /grant Everyone:F /T >nul 2>&1
